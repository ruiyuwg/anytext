...
7. Go to **Security** > **API Shield** > **Endpoint Management**.

8. Filter Endpoint Management by the `cf-risk-missing-auth` or `cf-risk-mixed-auth` labels.

9. Select an endpoint to see its authentication posture details on the endpoint details page.

10. Choose between the 24-hour and 7-day view options, and note any authentication changes over time.
