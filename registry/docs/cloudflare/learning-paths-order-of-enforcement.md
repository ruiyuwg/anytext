...
dns3\["Resolver policies (Enterprise users only)"]
...
http2\["Isolate policies  (with Browser Isolation add-on)"]
...
egress1\["Egress policies (Enterprise users only)"]
end
...
dns4 -----> internet
dns4 ---> cloudflare\["Private network services (Cloudflare Tunnel, Cloudflare WAN, WARP Connector)"]
