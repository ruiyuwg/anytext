...
CNI supports both Magic Transit (DDoS protection) and Cloudflare WAN (private networking). CNI also supports [BGP peering](https://developers.cloudflare.com/network-interconnect/get-started/) (beta) with the Cloudflare Virtual Network routing table for dynamic route exchange. When used with Magic Transit, cleaned inbound traffic always flows over CNI. Return traffic can either egress directly to the Internet (Direct Server Return, default) or route back through Cloudflare via [Magic Transit Egress](https://developers.cloudflare.com/magic-transit/reference/egress/).
...
| **Platform support**  | Linux only (x86\_64). Currently in beta.                                                            | Hardware appliance (Dell VEP1460) or virtual (VMware ESXi, Proxmox)                |
