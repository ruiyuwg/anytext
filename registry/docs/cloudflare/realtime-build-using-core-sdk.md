...
To integrate the Core SDK, you will need to initialize it with a [participant's auth token](https://developers.cloudflare.com/api/resources/realtime%5Fkit/#create-a-participant), and then use the provided SDK APIs to control the peer in the session.
...
Configure the meeting properties in the `RtkMeetingInfo` class with a valid participant `authToken` from the [Create Participant API](https://developers.cloudflare.com/api/resources/realtime%5Fkit/#create-a-participant).
