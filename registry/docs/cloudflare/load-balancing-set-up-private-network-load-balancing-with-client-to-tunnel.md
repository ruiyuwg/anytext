...
graph LR
W\[WARP clients] --> C{Private load balancer  100.112.0.0}
C -- Tunnel 1 --> cf1
C -- Tunnel 2 --> cf2
subgraph D2\[Data center 2]
cf2@{ shape: processes, label: "cloudflared" }
subgraph F\[Pool 2]
S3\["Endpoint  10.0.0.1 (VNET-2)"]
S4\["Endpoint  10.0.0.2 (VNET-2)"]
end
cf2-->S3
cf2-->S4
end
...
subgraph E\[Pool 1]
S1\["Endpoint  10.0.0.1 (VNET-1)"]
S2\["Endpoint  10.0.0.2 (VNET-1)"]
end
cf1-->S1
cf1-->S2
end
