...
A(DDoS  attack)
B\[("Cloudflare global  anycast network  (DDoS protection +  network firewall)")]
C\[Customer  network]
D((User))
E(\[BGP  announcement])
...
A --x B
E --- B
B-- Anycast  GRE tunnel ---C
B-- Cloudflare  Network  Interconnect ---C
C-- Egress through  Direct Server  Return --> D
D -- Ingress --> B
