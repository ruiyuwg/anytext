...
4. Under **When incoming requests match**, use the **Field** dropdown to choose *Bot Detection ID*. For **Operator**, select *equals*. For **Value**, enter the [bot detection ID](https://developers.cloudflare.com/browser-rendering/reference/automatic-request-headers/#bot-detection) for the method you want to allowlist.
