...
7. Choose a Network (`l4`) or HTTP (`http`) policy with an Allow action.

8. In the policy's [rule\_settings ↗](https://registry.terraform.io/providers/cloudflare/cloudflare/latest/docs/resources/zero%5Ftrust%5Fgateway%5Fpolicy), use the `check_session` argument to enable and configure a session timeout:
...
You can allow users to log in to Access applications using their device client session. WARP authentication is only supported for Access applications protected by Allow or Block policies.
