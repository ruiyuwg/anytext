...
This operation generates a data key pair through a random number generator, encrypts the data key pair using the primary version of a symmetric key, and returns the plaintext public key and ciphertext private key of the data key pair, without returning the plaintext private key. You can use this data key pair for signature verification outside Key Management Service (KMS).

Securely store the ciphertext private key (PrivateKeyCiphertextBlob), encryption algorithm (Algorithm), and authentication data (Aad). Use this saved information when you call the [AdvanceDecrypt](/help/en/kms/key-management-service/developer-reference/advancedecrypt) operation to decrypt the ciphertext private key.
