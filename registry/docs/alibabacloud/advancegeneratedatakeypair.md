...
This operation generates a data key pair by using a random number generator, encrypts the data key pair by using the **primary version** of a symmetric key, and returns the plaintext public key, plaintext private key, and ciphertext private key of the data key pair. You can use data key pairs to perform signature verification outside Key Management Service (KMS).

Store the ciphertext private key (PrivateKeyCiphertextBlob), encryption algorithm (Algorithm), and authentication data (Aad) in a secure location. The saved information is used when you call the [AdvanceDecrypt](/help/en/kms/key-management-service/developer-reference/advancedecrypt) operation to decrypt the ciphertext private key.
...
When you call the [Decrypt](/help/en/kms/key-management-service/developer-reference/decrypt-2#doc-api-Kms-Decrypt) operation to decrypt the data key pair, you must specify a valid value for Iv.
...
The plaintext private key of the data key pair.

-   If KeyFormat is set to DER in the request parameters, the PKCS#8 plaintext private key in the DER format is returned.
    
-   If KeyFormat is set to PEM in the request parameters, the PKCS#8 plaintext private key in the PEM format is returned.
