...
![image](https://help-static-aliyun-doc.aliyuncs.com/assets/img/en-US/1766595671/CAEQUBiBgMDl4u6s2RkiIDZlNjU3MThmMjg4ZTQxNmQ4N2YzNGVjYmEyZGMxNmY45148329_20250507114156.898.svg)
...
In a GDN, the [database proxy configuration](/help/en/polardb/polardb-for-postgresql/create-a-custom-cluster-endpoint) of each cluster determines how read and write requests are routed to the primary and secondary clusters. No application modifications are required. Simply connect to a cluster endpoint, and read and write requests are automatically routed as follows:
...
![image](https://help-static-aliyun-doc.aliyuncs.com/assets/img/en-US/2766595671/CAEQUBiBgICFyu.s2RkiIGVjMGQ2OWMzMDI5OTQ4ZTFhODM3YmUxMjA0NWIzMTYy5148329_20250507135141.418.svg)
...
![image](https://help-static-aliyun-doc.aliyuncs.com/assets/img/en-US/2766595671/CAEQUBiBgIDNmvCs2RkiIDZkOGFmOTI4MDllNTQxNjM4YjQxYmJmMWJkNjIzYTky5148329_20250507135141.418.svg)
...
-   Clusters in a GDN do not support the [serverless feature](/help/en/polardb/polardb-for-postgresql/serverless/) or [In-Memory Column Index (IMCI)](/help/en/polardb/polardb-for-postgresql/introduction-to-the-vectorization-engine/).
...
Cross-region data transfer for GDN is currently free of charge. You only need to pay the [fees](/help/en/polardb/polardb-for-postgresql/billable-items-3) for each PolarDB cluster.

## Quick start

1.  [Create a global database network](/help/en/polardb/polardb-for-postgresql/creating-and-deleting-a-global-database-network#title-2nv-njo-br3): You can select an [eligible](#2d5f22ee988hd) cluster to serve as the primary cluster of the GDN.
    
2.  [Add a secondary cluster](/help/en/polardb/polardb-for-postgresql/adding-and-managing-slave-clusters#title-gmh-abk-rgm): You can go to the [PolarDB purchase page](https://polardb-buy.alibabacloud.com/cusBuy/Prepaid) to add a secondary cluster to the GDN that you created.
    
    **Note**
    
    You can only create new secondary clusters. You cannot add an existing cluster as a secondary cluster.
    
3.  [Connect to the GDN](/help/en/polardb/polardb-for-postgresql/connect-to-the-global-database-network): In a GDN, each cluster (both primary and secondary) has an independent cluster endpoint. You can connect to the nearest cluster endpoint based on your application's region to access the database.
