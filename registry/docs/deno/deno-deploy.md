...
4. **GitHub repo** *(if source is GitHub)* - Select the owner and repo
...
```bash
deno deploy env update-contexts  [contexts...]
```
...
```bash
deno deploy database provision  --kind <denokv|prisma> [--region ]
```
...
```bash
deno deploy database link  [connectionString]
```
...
```bash
deno deploy database assign  [--app ]
```
...
```bash
deno deploy database detach  [--app ]
```
...
```bash
deno deploy database query   [query...]
```
