...
URL: https://docs.deno.com/subhosting/manual/api\_migration\_guide
...
| *(none)*  | `building`  | Build in progress     |
| `success` | `succeeded` | Deployed and live     |
| `failed`  | `failed`    | Build failed          |
| *(none)*  | `skipped`   | Build was skipped     |
...
| `order`  | *(none)* | v2 returns chronological order       |
| `q`      | `query`  | Text search                          |
| `level`  | `level`  | v1: `warning`. v2: `warn`            |
| `region` | *(none)* | Region filtering not available in v2 |
...
9. **Update field names** from camelCase to snake\_case.
