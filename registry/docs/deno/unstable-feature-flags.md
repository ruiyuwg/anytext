...
URL: https://docs.deno.com/runtime/reference/cli/unstable\_flags
...
1. The *package.json* has no `"type"` field.
2. No *package.json* exists.

By default, Deno only loads these modules as being possibly CommonJS when you're
in a project with a *package.json* and the closest *package.json* has
`{ "type": "commonjs" }`.
