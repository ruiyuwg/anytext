...
The `fields` property is an object of field types to allow your admin editors to build forms with. To override default settings, pass either a boolean value or a partial [Payload Block](../fields/blocks#block-configs) *keyed to the block's slug*. See [Fields](#fields) for more details.
...
is because *anyone* should be able to create a form submission, even from a
public-facing website, but *no one* should be able to update a submission once
...
Each field represents a form input. To override default settings pass either a boolean value or a partial [Payload Block](../fields/blocks) *keyed to the block's slug*. See [Field Overrides](#field-overrides) for more details on how to do this.

**Note:** "Fields" here is in reference to the *fields to build forms with*,
not to be confused with the *fields of a collection* which are set via
