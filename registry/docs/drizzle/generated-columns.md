...
\<Tabs items={\["PostgreSQL", "MySQL", "SQLite", "SingleStore(WIP)", "MSSQL", "CockroachDB"]}> <Tab>
\#### Database side
**Types**: `STORED` only
...
\<CodeTabs items={\["schema.ts"]}> <CodeTab>
...
``typescript copy
    ...
    ``
``sql
    CREATE TABLE `users` (       `id` int,       `id2` int,       `name` text,       `stored_gen` text GENERATED ALWAYS AS (concat(`users`.`name`, ' ', 'hello')) STORED,       `virtual_gen` text GENERATED ALWAYS AS (concat(`users`.`name`, ' ', 'hello')) VIRTUAL
    );
    `` </CodeTab> </CodeTabs> </Tab> <Tab>
\#### Database side
**Types**: `STORED`, `VIRTUAL`
...
``typescript copy
    ...
    ``
``sql
    CREATE TABLE `users` ( 	    `id` integer, 	    `name` text, 	    `stored_gen` text GENERATED ALWAYS AS ("name" || 'hello') STORED, 	    `virtual_gen` text GENERATED ALWAYS AS ("name" || 'hello') VIRTUAL
    );
    `` </CodeTab> </CodeTabs> </Tab> <Tab>
Work in Progress </Tab> <Tab>
\#### Database side
