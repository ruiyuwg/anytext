# Recommendations (/docs/optimize/recommendations)
...
Queries on unindexed columns \[#queries-on-unindexed-columns]
...
Indexing on unique columns \[#indexing-on-unique-columns]
...
Query pattern recommendations \[#query-pattern-recommendations]

Full table scans from LIKE operations \[#full-table-scans-from-like-operations]
...
Excessive rows returned \[#excessive-rows-returned]
...
Overfetching data \[#overfetching-data]
...
Data type recommendations \[#data-type-recommendations]

Avoid @db.Char(n) \[#avoid-dbcharn]
...
Avoid @db.VarChar(n) \[#avoid-dbvarcharn]
...
Avoid timestamp(0) and timestamptz(0) \[#avoid-timestamp0-and-timestamptz0]
...
Avoid CURRENT\_TIME \[#avoid-current\_time]
...
Storing BLOBs in database \[#storing-blobs-in-database]
...
- [Set up Optimize](/optimize/getting-started) to start receiving recommendations
- [Review query recordings](/optimize/recordings) to understand your application's query patterns
- [Use Prisma AI](/optimize/prisma-ai) to get personalized optimization suggestions
