...
| [Sandbox](/oss/python/deepagents/sandboxes)                      | `agent = create_deep_agent(backend=sandbox)` Execute code in isolated environments. Sandboxes provide filesystem tools plus the `execute` tool for running shell commands. Choose from Modal, Daytona, Deno, or local VFS.                                                                  |
