...
#### Send alerts to Slack
  ...
  **Request Body Template**
  It is required to fill in the `{channel_id}` from the value found in Step 4. The remaining fields: `alert_name`, `project_name` and `project_url` optionally add additional context to the alert message. You can find your `project_url` in the browser's URL bar. Copy the portion up to but not including any query parameters.
