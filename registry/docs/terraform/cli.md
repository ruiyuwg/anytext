...
- `terraform plan` starts a [speculative plan][] in an HCP Terraform workspace, using configuration files from a local directory. You can quickly check the results of edits (including compliance with Sentinel policies) without needing to copy sensitive variables to your local machine.
  ...
  Speculative plans work with all workspaces, and can co-exist with the [VCS-driven workflow](/terraform/cloud-docs/workspaces/run/ui).
...
- No Terraform variables or environment variables are created by default, unless your organization has configured one or more [global variable sets](/terraform/cloud-docs/variables#scope). HCP Terraform will use `*.auto.tfvars` files if they are present, but you will usually still need to set some workspace-specific variables.
...
Refer to [Variables](/terraform/cloud-docs/variables) for more details about variable types, variable scopes, variable precedence, and how to set run-specific variables through the command line.
...
In workspaces that are not connected to a VCS repository, users with [permission to apply runs](/terraform/cloud-docs/users-teams-organizations/permissions/workspace) can use the CLI to trigger remote applies. Remote applies use the configuration code from the local working directory, but use the variable values from the specified workspace.
...
External systems cannot run the traditional apply workflow because Terraform requires console input from the user to approve plans. We recommend using the [API-driven Run Workflow](/terraform/cloud-docs/workspaces/run/api) for non-interactive workflows when possible.
...
[Run Modes and Options](/terraform/cloud-docs/workspaces/run/modes-and-options) contains more details about the various options available for plans and applies when you use the CLI-driven workflow.
