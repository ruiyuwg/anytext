...
These instructions describe how to connect to GitLab.com. Refer to [GitLab EE](/terraform/cloud-docs/vcs/gitlab-eece) for instructions on connecting to GitLab EE. Refer to [VCS overview](/terraform/cloud-docs/vcs) for information about abll VCS integrations.
...
Alternately, you can skip the OAuth configuration process and authenticate with a personal access token. This requires using HCP Terraform's API. For details, see [the OAuth Clients API page](/terraform/cloud-docs/api-docs/oauth-clients).

## Requirements

Configuring a new VCS provider requires permission to manage VCS settings for the organization. Refer to [Permissions](/terraform/cloud-docs/users-teams-organizations/permissions) for more information.
