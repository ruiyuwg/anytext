---
page_title: /policy-checks API reference for HCP Terraform
description: >-
  Use the HCP Terraform API's `/policy-checks` endpoint to manage and override the Sentinel policy checks that HCP Terraform performs during a run.
...
Policy checks are the default workflow for Sentinel evaluating plans on workspaces. Policy checks use the latest version of the Sentinel runtime and have access to cost estimation data.
