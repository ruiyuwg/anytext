...
#### **Create an HTML template** with a \`\` comment where server content goes
...
The `index.html` file contains an \`\` comment that marks where server-rendered content will be inserted. Nitro replaces this comment with the output from your server entry.
