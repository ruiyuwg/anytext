...
The default production output preset is [Node.js server](https://nitro.build/deploy/runtimes/node).
...
Other built-in providers are available with an explicit preset, including [zephyr](https://nitro.build/deploy/providers/zephyr).

## Changing the deployment preset

If you need to build Nitro against a specific provider, you can target it by defining an environment variable named `NITRO_PRESET` or `SERVER_PRESET`, or by updating your Nitro [configuration](https://nitro.build/docs/configuration) or using `--preset` argument.
