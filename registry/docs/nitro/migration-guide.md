...
The NPM package [nitropack](https://www.npmjs.com/package/nitropack){rel=""nofollow""} (v2) has been renamed to [nitro](https://www.npmjs.com/package/nitro){rel=""nofollow""} (v3).
...
Nitro now requires a minimum Node.js version of 20, as Node.js 18 reaches end-of-life in [April 2025](https://nodejs.org/en/about/previous-releases){rel=""nofollow""}.

Please upgrade to the [latest LTS](https://nodejs.org/en/download){rel=""nofollow""} version (>= 20).
...
Nitro v3 upgrades to [H3 v2](https://h3.dev){rel=""nofollow""}, which includes API changes. All H3 utilities are imported from `nitro/h3`.
...
Access to `event.node.{req,res}` is only available in Node.js runtime. `event.web` is renamed to `event.req` (instance of web [`Request`](https://developer.mozilla.org/en-US/docs/Web/API/Request){rel=""nofollow""}).
...
H3 now uses standard web [`Headers`](https://developer.mozilla.org/en-US/docs/Web/API/Headers){rel=""nofollow""}. Header values are always plain `string` (no `null`, `undefined`, or `string[]`).
