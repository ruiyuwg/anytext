...
#### Saner default values[​](#saner-default-values "Direct link to Saner default values")
...
#### CORS handling[​](#cors-handling "Direct link to CORS handling")
...
#### No more cookie by default[​](#no-more-cookie-by-default "Direct link to No more cookie by default")
...
#### io.set() is removed[​](#ioset-is-removed "Direct link to io.set() is removed")
...
#### No more implicit connection to the default namespace[​](#no-more-implicit-connection-to-the-default-namespace "Direct link to No more implicit connection to the default namespace")
...
#### Namespace.connected is renamed to Namespace.sockets and is now a Map[​](#namespaceconnected-is-renamed-to-namespacesockets-and-is-now-a-map "Direct link to Namespace.connected is renamed to Namespace.sockets and is now a Map")
...
#### Socket.rooms is now a Set[​](#socketrooms-is-now-a-set "Direct link to Socket.rooms is now a Set")
...
#### Socket.binary() is removed[​](#socketbinary-is-removed "Direct link to Socket.binary() is removed")
...
#### Socket.join() and Socket.leave() are now synchronous[​](#socketjoin-and-socketleave-are-now-synchronous "Direct link to Socket.join() and Socket.leave() are now synchronous")
...
#### Socket.use() is removed[​](#socketuse-is-removed "Direct link to socketuse-is-removed")
...
#### A middleware error will now emit an Error object[​](#a-middleware-error-will-now-emit-an-error-object "Direct link to A middleware error will now emit an Error object")
...
#### Add a clear distinction between the Manager query option and the Socket query option[​](#add-a-clear-distinction-between-the-manager-query-option-and-the-socket-query-option "Direct link to Add a clear distinction between the Manager query option and the Socket query option")
...
#### The Socket instance will no longer forward the events emitted by its Manager[​](#the-socket-instance-will-no-longer-forward-the-events-emitted-by-its-manager "Direct link to The Socket instance will no longer forward the events emitted by its Manager")
...
#### Namespace.clients() is renamed to Namespace.allSockets() and now returns a Promise[​](#namespaceclients-is-renamed-to-namespaceallsockets-and-now-returns-a-promise "Direct link to Namespace.clients() is renamed to Namespace.allSockets() and now returns a Promise")
...
#### Client bundles[​](#client-bundles "Direct link to Client bundles")
...
#### No more "pong" event for retrieving latency[​](#no-more-pong-event-for-retrieving-latency "Direct link to No more \"pong\" event for retrieving latency")
...
#### ES modules syntax[​](#es-modules-syntax "Direct link to ES modules syntax")
...
#### `emit()` chains are not possible anymore[​](#emit-chains-are-not-possible-anymore "Direct link to emit-chains-are-not-possible-anymore")
...
#### Room names are not coerced to string anymore[​](#room-names-are-not-coerced-to-string-anymore "Direct link to Room names are not coerced to string anymore")
...
#### Volatile events (client)[​](#volatile-events-client "Direct link to Volatile events (client)")
...
#### Official bundle with the msgpack parser[​](#official-bundle-with-the-msgpack-parser "Direct link to Official bundle with the msgpack parser")
...
#### The Socket.IO codebase has been rewritten to TypeScript[​](#the-socketio-codebase-has-been-rewritten-to-typescript "Direct link to The Socket.IO codebase has been rewritten to TypeScript")
...
#### Support for IE8 and Node.js 8 is officially dropped[​](#support-for-ie8-and-nodejs-8-is-officially-dropped "Direct link to Support for IE8 and Node.js 8 is officially dropped")
...
### How to upgrade an existing production deployment[​](#how-to-upgrade-an-existing-production-deployment "Direct link to How to upgrade an existing production deployment")
...
### Known migration issues[​](#known-migration-issues "Direct link to Known migration issues")
...
-   [`cors`](/docs/v4/server-api/#cors) option
-   [`withCredentials`](/docs/v4/client-api/#withcredentials) option
