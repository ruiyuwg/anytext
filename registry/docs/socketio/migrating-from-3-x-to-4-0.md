...
Please note that these breaking changes only impact the API on the server side. The Socket.IO protocol itself was not updated, so a v3 client **will** be able to reach a v4 server and vice-versa. Besides, the compatibility mode ([`allowEIO3: true`](/docs/v4/server-options/#alloweio3)) is still available between a Socket.IO v2 client and a Socket.IO v4 server.
...
#### `io.to()` is now immutable[​](#ioto-is-now-immutable "Direct link to ioto-is-now-immutable")
...
#### `wsEngine` option[​](#wsengine-option "Direct link to wsengine-option")

The format of the [`wsEngine`](/docs/v4/server-options/#wsengine) option was updated in order to get rid of the following error:
...
#### Ensure compatibility with Swift v15 clients[​](#ensure-compatibility-with-swift-v15-clients "Direct link to Ensure compatibility with Swift v15 clients")
...
That's why a Swift client v15 was not able to connect to the server, even when the compatibility mode was enabled ([`allowEIO3: true`](/docs/v4/server-options/#alloweio3)), unless you explicitly specified the query param:
...
#### The default value of `pingTimeout` was increased[​](#the-default-value-of-pingtimeout-was-increased "Direct link to the-default-value-of-pingtimeout-was-increased")

The default value of [`pingTimeout`](/docs/v4/server-options/#pingtimeout) (used in the [heartbeat mechanism](/docs/v4/how-it-works/#disconnection-detection)) value was updated from 60000 to 5000 in `socket.io@2.1.0` (March 2018).
...
#### Allow excluding specific rooms when broadcasting[​](#allow-excluding-specific-rooms-when-broadcasting "Direct link to Allow excluding specific rooms when broadcasting")
...
#### Allow to pass an array to `io.to()`[​](#allow-to-pass-an-array-to-ioto "Direct link to allow-to-pass-an-array-to-ioto")
...
#### Additional utility methods[​](#additional-utility-methods "Direct link to Additional utility methods")
...
#### Typed events[​](#typed-events "Direct link to Typed events")
...
#### `autoUnref` option[​](#autounref-option "Direct link to autounref-option")
