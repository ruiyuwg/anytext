...
## Types for the server[​](#types-for-the-server "Direct link to Types for the server")
...
```
interface ServerToClientEvents {  // [...]  withAck: (d: string, callback: (result: number) => void) => void;}try {  const result = await socket.timeout(5_000).emitWithAck("withAck", "some string");  // result is inferred as <number>} catch (err) {  // handle error}
```
...
## Types for the client[​](#types-for-the-client "Direct link to Types for the client")
...
## Custom types for each namespace[​](#custom-types-for-each-namespace "Direct link to Custom types for each namespace")

Since each [Namespace](/docs/v4/namespaces/) can have its own set of events, you can also provide some types for each one of them:
