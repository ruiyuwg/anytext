...
## Additional packages[​](#additional-packages "Direct link to Additional packages")
...
## Other WebSocket server implementations[​](#other-websocket-server-implementations "Direct link to Other WebSocket server implementations")
...
## Usage with `µWebSockets.js`[​](#usage-with-uwebsockets "Direct link to usage-with-uwebsockets")
...
## Usage with Bun[​](#usage-with-bun "Direct link to Usage with Bun")
...
```
import { Server as Engine } from "@socket.io/bun-engine";import { Server } from "socket.io";const io = new Server();const engine = new Engine({  path: "/socket.io/",});io.bind(engine);io.on("connection", (socket) => {  // ...});export default {  port: 3000,  idleTimeout: 30, // must be greater than the "pingInterval" option of the engine, which defaults to 25 seconds  ...engine.handler(),};
```
...
See also: [https://github.com/microsoft/types-publisher/issues/81#issuecomment-234051345](https://github.com/microsoft/types-publisher/issues/81#issuecomment-234051345)

### Transitive versions[​](#transitive-versions "Direct link to Transitive versions")

The `engine.io` package brings the engine that is responsible for managing the low-level connections (HTTP long-polling or WebSocket). See also: [How it works](/docs/v4/how-it-works/)
