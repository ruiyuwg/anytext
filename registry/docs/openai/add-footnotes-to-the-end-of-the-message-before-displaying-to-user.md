...
python: `run = client.beta.threads.runs.create(
  thread_id=thread.id,
  assistant_id=assistant.id
)
 `.trim(),
"node.js": `const run = await openai.beta.threads.runs.create(
  thread.id,
  { assistant_id: assistant.id }
);
 `.trim(),
curl: `curl https://api.openai.com/v1/threads/THREAD_ID/runs \\
...
| `in_progress`     | While in\_progress, the Assistant uses the model and tools to perform steps. You can view progress being made by the Run by examining the [Run Steps](https://developers.openai.com/api/docs/api-reference/runs/step-object).                                                                                                                                                                                                                                                                                                            |
