...
The `createMemo` function creates a read-only signal that derives its value from other reactive values. The calculated value is memoized: the calculation runs only when dependencies change, and is reused when the value is read. When a dependency changes, the calculation re-executes. If the new result is equal to the previous result (according to the [`equals`](/reference/basic-reactivity/create-memo#equals) option), the memo suppresses downstream updates.

***

## [Import](/reference/basic-reactivity/create-memo#import)
...
## [Type](/reference/basic-reactivity/create-memo#type)

```
function createMemo<T>(  fn: (v: T) => T,  value?: T,  options?: { equals?: false | ((prev: T, next: T) => boolean); name?: string }): () => T;
```

***

## [Parameters](/reference/basic-reactivity/create-memo#parameters)

### [`fn`](/reference/basic-reactivity/create-memo#fn)
...
### [`value`](/reference/basic-reactivity/create-memo#value)
...
### [`options`](/reference/basic-reactivity/create-memo#options)
...
#### [`equals`](/reference/basic-reactivity/create-memo#equals)
...
#### [`name`](/reference/basic-reactivity/create-memo#name)
...
## [Return value](/reference/basic-reactivity/create-memo#return-value)
...
## [Examples](/reference/basic-reactivity/create-memo#examples)

### [Basic usage](/reference/basic-reactivity/create-memo#basic-usage)
...
function FilterList() {  const [query, setQuery] = createSignal("");
  ...
  return (    <div>      <input        value={query()}        onInput={(e) => setQuery(e.currentTarget.value)}        placeholder="Search..."      />
      ...
      <ul>        {/* Accessing the memo again does not trigger re-execution. */}        <For each={filteredNames()}>{(name) => <li>{name}</li>}</For>      </ul>    </div>  );}
```

### [Custom equality check](/reference/basic-reactivity/create-memo#custom-equality-check)
...
function DateNormalizer() {  const [dateString, setDateString] = createSignal("2024-05-10");
  ...
  return (    <div>      <input        value={dateString()}        onInput={(e) => setDateString(e.currentTarget.value)}      />      {/* Setting the same date string creates a new Date object,          but `equals` prevents the update propagation. */}      <button onClick={() => setDateString("2024-05-10")}>        Reset to the same date      </button>    </div>  );}
```

### [Accessing previous value](/reference/basic-reactivity/create-memo#accessing-previous-value)
