...
- **Automatic data revalidation:** After an action successfully completes, Solid Router revalidates relevant [`queries`](/solid-router/data-fetching/queries), ensuring the UI reflects the latest data.
- **Progressive enhancement:** When used with HTML forms, actions enable functionality even if JavaScript is not yet loaded.

***

## [Defining actions](/solid-router/concepts/actions#defining-actions)
...
### [With the `<form>` element](/solid-router/concepts/actions#with-the-form-element)
...
```
<form action={uploadFileAction} method="post" enctype="multipart/form-data">  <input type="file" name="myFile" />  <button type="submit">Upload</button></form>
```

#### [Passing additional arguments](/solid-router/concepts/actions#passing-additional-arguments)
...
### [With the `useAction` primitive](/solid-router/concepts/actions#with-the-useaction-primitive)
...
## [Tracking submission state](/solid-router/concepts/actions#tracking-submission-state)

When an action is triggered, it creates a **submission** object. This object is a snapshot of the action's execution, containing its input, current status (pending or complete), and its final result or error. To access this state, Solid Router provides the [`useSubmission`](/solid-router/reference/data-apis/use-submission) and [`useSubmissions`](/solid-router/reference/data-apis/use-submissions) primitives.
...
## [Handling errors](/solid-router/concepts/actions#handling-errors)
...
It is important that an action consistently returns a value from all of its possible code paths. Because, if an action returns `undefined` or `null`, Solid Router removes that submission from its internal list upon completion. This can lead to unexpected behavior.

For example, consider an action that returns an error object on failure but returns nothing on success. If the action fails once, `useSubmission` will correctly report the error. However, if a subsequent submission succeeds, it will be removed from the list, and `useSubmission` will continue to report the previous stale error state. To prevent this, ensure every code path in an action returns a value, such as `{ ok: true }` to indicate a successful outcome.

***

## [Automatic data revalidation](/solid-router/concepts/actions#automatic-data-revalidation)
...
## [Managing navigation and revalidation](/solid-router/concepts/actions#managing-navigation-and-revalidation)
...
### [Redirecting](/solid-router/concepts/actions#redirecting)

To navigate the user to a new page after an action completes, the [`redirect` helper](/solid-router/reference/response-helpers/redirect) can be used. It can also be used to revalidate specific queries upon redirection, which is useful for updating data that is displayed on the new page.
...
### [Customizing revalidation](/solid-router/concepts/actions#customizing-revalidation)
...
A response helper can be either `return`ed or `throw`n. In TypeScript, `throw` can be more convenient, as it avoids potential type conflicts with an action's expected return value.

***

## [Optimistic UI](/solid-router/concepts/actions#optimistic-ui)
...
For more advanced patterns, consider using [TanStack Query](https://tanstack.com/query/latest/docs/framework/solid/guides/optimistic-updates). It provides robust tools for managing server state, including cache-based optimistic updates.
...
1. [Overview](#_top)
2. [Defining actions](#defining-actions)
3. [Using actions](#using-actions)
   1. [With the  element](#with-the-form-element)
   2. [With the useAction primitive](#with-the-useaction-primitive)
4. [Tracking submission state](#tracking-submission-state)
5. [Handling errors](#handling-errors)
6. [Automatic data revalidation](#automatic-data-revalidation)
7. [Managing navigation and revalidation](#managing-navigation-and-revalidation)
   1. [Redirecting](#redirecting)
   2. [Customizing revalidation](#customizing-revalidation)
8. [Optimistic UI](#optimistic-ui)

Contribute

1. [Edit this page](https://github.com/solidjs/solid-docs/edit/main/src/routes/solid-router/concepts/actions.mdx)
2. [Report an issue with this page](https://github.com/solidjs/solid-docs-next/issues/new?assignees=ladybluenotes\&labels=improve+documentation%2Cpending+review\&projects=\&template=CONTENT.yml\&title=[Content]:\&subject=/solid-router/concepts/actions.mdx\&page=https://docs.solidjs.com/solid-router/concepts/actions)
