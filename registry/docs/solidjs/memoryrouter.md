...
## [Manipulating the history](/solid-router/reference/components/memory-router#manipulating-the-history)
...
## [Properties](/solid-router/reference/components/memory-router#properties)

### [`MemoryHistory`](/solid-router/reference/components/memory-router#memoryhistory)
...
Navigates to a new route. `value`: URL to navigate to. `scroll`: Scrolls to element by ID (default: `false`). `replace`: Replaces the current history entry (default: `false`).
...
### [`MemoryRouter`](/solid-router/reference/components/memory-router#memoryrouter)
...
Disables all anchors being intercepted and instead requires `<A>`. default: `false`
