...
# Interface ListCertificatesRequestOrBuilder (2.24.0) Stay organized with collections Save and categorize content based on your preferences.
...
```
public interface ListCertificatesRequestOrBuilder extends MessageOrBuilder
```
...
Optional. Limit on the number of Certificates to include in the response. Further Certificates can subsequently be obtained by including the ListCertificatesResponse.next\_page\_token in a subsequent request. If unspecified, the server will pick an appropriate default.
...
Optional. Pagination token, returned earlier via ListCertificatesResponse.next\_page\_token.
