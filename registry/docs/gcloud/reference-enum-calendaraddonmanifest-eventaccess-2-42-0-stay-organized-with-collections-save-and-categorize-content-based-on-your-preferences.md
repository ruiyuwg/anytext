...
# Enum CalendarAddOnManifest.EventAccess (2.42.0) Stay organized with collections Save and categorize content based on your preferences.
...
```
public enum CalendarAddOnManifest.EventAccess extends Enum<CalendarAddOnManifest.EventAccess> implements ProtocolMessageEnum
```

An enum defining the level of data access event triggers require.

Protobuf enum `google.apps.script.type.calendar.CalendarAddOnManifest.EventAccess`
...
READ\_WRITE gives event triggers access to all provided event fields including the metadata, attendees, and conference data and the ability to perform all actions.
...
**Deprecated.** _Use [#forNumber(int)](/java/docs/reference/google-cloud-gsuite-addons/2.42.0/com.google.apps.script.type.calendar.CalendarAddOnManifest.EventAccess#com_google_apps_script_type_calendar_CalendarAddOnManifest_EventAccess_forNumber_int_) instead._
