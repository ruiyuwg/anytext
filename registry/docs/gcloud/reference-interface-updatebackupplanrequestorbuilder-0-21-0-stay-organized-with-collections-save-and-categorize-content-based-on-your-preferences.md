...
# Interface UpdateBackupPlanRequestOrBuilder (0.21.0) Stay organized with collections Save and categorize content based on your preferences.
...
```
public interface UpdateBackupPlanRequestOrBuilder extends MessageOrBuilder
```
...
`[BackupPlan](/java/docs/reference/google-cloud-gke-backup/0.21.0/com.google.cloud.gkebackup.v1.BackupPlan)`
...
`[BackupPlanOrBuilder](/java/docs/reference/google-cloud-gke-backup/0.21.0/com.google.cloud.gkebackup.v1.BackupPlanOrBuilder)`
...
This is used to specify the fields to be overwritten in the BackupPlan targeted for update. The values for each of these updated fields will be taken from the `backup_plan` provided with this request. Field names are relative to the root of the resource (e.g., `description`, `backup_config.include_volume_data`, etc.) If no `update_mask` is provided, all fields in `backup_plan` will be written to the target BackupPlan resource. Note that OUTPUT\_ONLY and IMMUTABLE fields in `backup_plan` are ignored and are not used to update the target BackupPlan.
