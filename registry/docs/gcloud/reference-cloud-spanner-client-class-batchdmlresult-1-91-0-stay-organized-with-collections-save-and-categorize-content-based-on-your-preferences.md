...
# Cloud Spanner Client - Class BatchDmlResult (1.91.0) Stay organized with collections Save and categorize content based on your preferences.
...
If an error occurred, the method returns an array, where the `status` key contains error information, represented as [google.rpc.Status](https://cloud.google.com/spanner/docs/reference/rpc/google.rpc#status), and the 'statement\` key contains the input which caused the error.
