...
# Interface DeleteAwsNodePoolRequestOrBuilder (0.12.0) Stay organized with collections Save and categorize content based on your preferences.
...
```
public interface DeleteAwsNodePoolRequestOrBuilder extends MessageOrBuilder
```
...
Required. The resource name the AwsNodePool to delete. `AwsNodePool` names are formatted as `projects/<project-id>/locations/<region>/awsClusters/<cluster-id>/awsNodePools/<node-pool-id>`. See [Resource Names](https://cloud.google.com/apis/design/resource_names) for more details on Google Cloud resource names.
