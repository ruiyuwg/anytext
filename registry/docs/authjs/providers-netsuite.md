[API reference](/reference/overview "API reference")[@auth/core](/reference/core "@auth/core")[providers](/reference/core/providers "providers")netsuite

# providers/netsuite

Built-in **NetSuite** integration.[![](https://authjs.dev/img/providers/netsuite.svg)](https://system.netsuite.com)

## NetSuiteProfile[](#netsuiteprofile)
...
## OAuthNetSuiteOptions[](#oauthnetsuiteoptions)
...
[https://docs.oracle.com/en/cloud/saas/netsuite/ns-online-help/section\_160855585734.html](https://docs.oracle.com/en/cloud/saas/netsuite/ns-online-help/section_160855585734.html)
...
restlets rest\_webservices or restlets or rest\_webservices suiteanalytics\_connect restlets
...
When setting the Redirect URI in the Integration record, you must use the `https` protocol. Otherwise, you will get an error when trying to sign in. (_INVALID\_LOGIN\_ATTEMPT_). If you are testing locally, you can use a service like [ngrok](https://ngrok.com/) to create a secure tunnel to your localhost.
...
Our `userinfo` needs to compose of a suitelet or RESTLet url that gives us the information about the user. This has to be very fast in which the handshake profile gather execution can’t take long. The best bet is to use the `N/runtime` module to get the basics first. - Here is an example of a RESTlet below. Be sure to deploy and enable access to “All Roles”.

#### Example RESTLet Callback Handler[](#example-restlet-callback-handler)
...
Make sure the `userinfo` matches the return type of the profile callback to ensure the user session gets read correctly. To override the defaults for your use case, check out [customizing a built-in OAuth provider](https://authjs.dev/guides/providers/custom-provider#override-default-options).

### Type Parameters[](#type-parameters)

Type Parameter

`P` _extends_ [`NetSuiteProfile`](netsuite#netsuiteprofile)
...
[`OAuthUserConfig`](../providers#oauthuserconfigprofile)<`P`\> & [`OAuthNetSuiteOptions`](netsuite#oauthnetsuiteoptions)

### Returns[](#returns)

[`OAuthConfig`](../providers#oauthconfigprofile)<`P`\>

[netlify](/reference/core/providers/netlify "netlify")[nextcloud](/reference/core/providers/nextcloud "nextcloud")
