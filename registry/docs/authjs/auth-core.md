[API reference](/reference/overview "API reference")@auth/core
...
{ `account`: `null` | [`Account`](core/types#account); `isNewUser`: `boolean`; `profile`: [`Profile`](core/types#profile); `session`: `any`; `token`: [`JWT`](core/jwt#jwt); `trigger`: `"signIn"` | `"signUp"` | `"update"`; `user`: | [`User`](core/types#user-2) | [`AdapterUser`](core/adapters#adapteruser); }

\-

`params.account`?

`null` | [`Account`](core/types#account)
...
| [`User`](core/types#user-2) | [`AdapterUser`](core/adapters#adapteruser)
...
{ `session`: { `user`: [`AdapterUser`](core/adapters#adapteruser); } & [`AdapterSession`](core/adapters#adaptersession); `user`: [`AdapterUser`](core/adapters#adapteruser); } & { `session`: [`Session`](core/types#session); `token`: [`JWT`](core/jwt#jwt); } & { `newSession`: `any`; `trigger`: `"update"`; }

###### Returns[](#returns-2)

[`Awaitable`](core/types#awaitablet)< | [`Session`](core/types#session) | [`DefaultSession`](core/types#defaultsession)\>
...
{ `account`: `null` | [`Account`](core/types#account); `credentials`: [`Record`](https://www.typescriptlang.org/docs/handbook/utility-types.html#recordkeys-type)<`string`, [`CredentialInput`](core/providers/credentials#credentialinput)\>; `email`: { `verificationRequest`: `boolean`; }; `profile`: [`Profile`](core/types#profile); `user`: | [`User`](core/types#user-2) | [`AdapterUser`](core/adapters#adapteruser); }
...
{ `user`: [`User`](core/types#user-2); }

`message.user`

[`User`](core/types#user-2)
...
{ `account`: [`Account`](core/types#account); `profile`: | [`User`](core/types#user-2) | [`AdapterUser`](core/adapters#adapteruser); `user`: | [`User`](core/types#user-2) | [`AdapterUser`](core/adapters#adapteruser); }

`message.account`

[`Account`](core/types#account)
...
{ `session`: [`Session`](core/types#session); `token`: [`JWT`](core/jwt#jwt); }

`message.session`

[`Session`](core/types#session)
...
{ `account`: `null` | [`Account`](core/types#account); `isNewUser`: `boolean`; `profile`: [`Profile`](core/types#profile); `user`: [`User`](core/types#user-2); }
...
Auth.js returns a web standard [Response](https://developer.mozilla.org/docs/Web/API/Response) by default, but if you are implementing a framework you might want to get access to the raw internal response by passing this value to [AuthConfig.raw](core#raw).

* * *

## skipCSRFCheck[](#skipcsrfcheck-1)
...
Auth.js comes with built-in CSRF protection, but if you are implementing a framework that is already protected against CSRF attacks, you can skip this check by passing this value to [AuthConfig.skipCSRFCheck](core#skipcsrfcheck).
...
[`AuthConfig`](core#authconfig) & { `raw`: _typeof_ [`raw`](core#raw-1); }

#### Returns[](#returns-11)

[`Promise`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Promise)<[`ResponseInternal`](core/types#responseinternalbody)<`any`\>>
...
[`AuthAction`](core/types#authaction)
...
[`Pick`](https://www.typescriptlang.org/docs/handbook/utility-types.html#picktype-keys)<[`AuthConfig`](core#authconfig), `"logger"` | `"basePath"`\>
...
[Overview](/reference/overview "Overview")[adapters](/reference/core/adapters "adapters")
