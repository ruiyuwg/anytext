[Guides](/guides/debugging "Guides")Extending the Session
...
This will add the user’s id to the session object. Notice that in this case, we are getting the id from the `user` object, not the `token`. With the database session strategy, the `user` object is the user from the database, and there is no `token`.
...
## With provider functions[](#with-provider-functions)
...
[Environment Variables](/guides/environment-variables "Environment Variables")[Restricting users accessing to the app](/guides/restricting-user-access "Restricting users accessing to the app")
