[Guides](/guides/debugging "Guides")Creating a Database Adapter
...
### Methods and models[](#methods-and-models-1)
...
### Methods and models[](#methods-and-models-2)
...
## Official adapter guidelines[](#official-adapter-guidelines)
...
2.  [Adapter tests](https://github.com/nextauthjs/next-auth/blob/main/packages/utils/adapter.ts) _must_ be included and _must_ pass. Docker is favored over services, to make CI resilient to network errors and to reduce the number of GitHub Action Secrets (which also lets us run these tests in fork PRs)
    
3.  The Adapter _must_ follow these coding styles
    ...
    -   collection/table names should follow the convention (plural/singular, camelCase/snake\_case) of the underlying ORM/database docs/conventions
...
5.  The Adapter _must_ be able to handle any property coming from the user
...
[Configuring Custom HTTP Email Provider](/guides/configuring-http-email "Configuring Custom HTTP Email Provider")[Creating a Framework Integration](/guides/creating-a-framework-integration "Creating a Framework Integration")
