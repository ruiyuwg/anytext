[Getting Started](/getting-started "Getting Started")[Adapters](/getting-started/adapters/azure-tables "Adapters")Dgraph

![](/img/adapters/dgraph.svg)
...
### Unsecure Schema[](#unsecure-schema)
...
### Dgraph.Authorization[](#dgraphauthorization)
...
### VerificationKey and jwtSecret[](#verificationkey-and-jwtsecret)

This is the key used to sign the JWT. Ex. `process.env.SECRET` or `process.env.APP_SECRET`.

### Header and authHeader[](#header-and-authheader)
...
### The nextAuth secret[](#the-nextauth-secret)
...
### JWT session and `@auth` directive[](#jwt-session-and-auth-directive)

Dgraph only works with HS256 or RS256 algorithms. If you want to use session jwt to securely interact with your dgraph database you must customize next-auth `encode` and `decode` functions, as the default algorithm is HS512. You can further customize the jwt with roles if you want to implement [`RBAC logic`](https://dgraph.io/docs/graphql/authorization/directive/#role-based-access-control).
...
[Cloudflare D1](/getting-started/adapters/d1 "Cloudflare D1")[Drizzle](/getting-started/adapters/drizzle "Drizzle")
