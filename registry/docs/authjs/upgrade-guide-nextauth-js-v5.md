[Getting Started](/getting-started "Getting Started")Migrating to v5

# Upgrade Guide (NextAuth.js v5)

This guide only applies to `next-auth` upgrades for users of Next.js. Feel free to skip to the next section, [Installation](/getting-started/installation), if you are not upgrading to `next-auth@5`.
...
## Authenticating server-side[](#authenticating-server-side)
...
`withAuth(middleware, subset of authOptions)` wrapper

`auth` export / `auth()` wrapper
...
`getToken(req)` (No session rotation)
...
`getToken(ctx.req)` (No session rotation)

`auth(req, res)` call

#### Details[](#details)

Server Component (App)Client Component (App)MiddlewareAPI Routes (Pages)getServerSideProps (Pages)
...
proxy.ts (middleware.ts in Next.js < 16)
...
Whenever `auth()` is passed the res object, it will rotate the session expiry. This was not the case with `getToken()` previously. The default session expiry is 30 days, but you can change it by setting `authOptions.session.maxAge`.
...
Check out the [adapters page](/getting-started/database) for a list of official adapters, or the “[create a database adapter](/guides/creating-a-database-adapter)” guide to learn how to create your own.
...
Furthermore, previously uncommon fields like GitHub’s `refresh_token_expires_in` fields were required to be added to the `account` table. This is no longer the case, and you can remove it if you are not using it. If you do use it, please make sure to return it via the new [`account()` callback](/reference/core/providers#account).
...
[Integrations](/getting-started/integrations "Integrations")
