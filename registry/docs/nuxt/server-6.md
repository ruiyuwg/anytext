...
:read-more{target="\_blank" title="Nitro Plugins" to="https://nitro.build/guide/plugins"}

## Server Utilities

Server routes are powered by [h3js/h3](https://github.com/h3js/h3){rel=""nofollow""} which comes with a handy set of helpers.

:read-more{target="\_blank" title="Available H3 Request Helpers" to="https://www.jsdocs.io/package/h3#package-index-functions"}
...
:read-more{to="https://nuxt.com/docs/3.x/guide/concepts/server-engine"}
...
Nitro provides a cross-platform [storage layer](https://nitro.build/guide/storage){rel=""nofollow""}. In order to configure additional storage mount points, you can use `nitro.storage`, or [server plugins](https://nuxt.com/docs/3.x/directory-structure/server#server-plugins).
