...
If runtime context is unavailable in your scope, `useNuxtApp` will throw an exception when called. You can use [`tryUseNuxtApp`](https://nuxt.com/docs/3.x/api/composables/use-nuxt-app#tryusenuxtapp) instead for composables that do not require `nuxtApp`, or to simply check if context is available or not without an exception.
...
- [`use()`](https://vuejs.org/api/application#app-use){rel=""nofollow""} - Installs a \*\*[Vue.js Plugin](https://vuejs.org/guide/reusability/plugins){rel=""nofollow""}\*\* [(example)](https://nuxt.com/docs/3.x/directory-structure/plugins#vue-plugins).
...
- `state` (object) - When you use [`useState`](https://nuxt.com/docs/3.x/api/composables/use-state) composable in Nuxt to set shared state, this state data is accessed through `payload.state.[name-of-your-state]`.
  ...
  ```
  :brIt is also possible to use more advanced types, such as `ref`, `reactive`, `shallowRef`, `shallowReactive` and `NuxtError`.
...
:read-more{to="https://nuxt.com/docs/3.x/guide/going-further/experimental-features#asynccontext"}
