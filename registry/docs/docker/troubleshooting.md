...
- Ubuntu 24.04 and later enables restricted unprivileged user namespaces by default, which prevents unprivileged processes in creating user namespaces unless an AppArmor profile is configured to allow programs to use unprivileged user namespaces.
  ...
  1. Create and install the currently logged-in user's AppArmor profile:
     ...
     $ [ ! -z "${filename}" ] && sudo cat < /etc/apparmor.d/${filename}
     abi <abi/4.0>,
...
Default when `slirp4netns` isn't installed
...
Experimental; Needs pasta version 2023\_12\_04 or later
...
**Note:** Not integrated to RootlessKit as it needs a custom seccomp profile
...
[Edit this page](https://github.com/docker/docs/edit/main/content/manuals/engine/security/rootless/troubleshoot.md)

[Request changes](https://github.com/docker/docs/issues/new?template=doc_issue.yml\&location=https%3a%2f%2fdocs.docker.com%2fengine%2fsecurity%2frootless%2ftroubleshoot%2f\&labels=status%2Ftriage)
