...
Applies the logical AND operation to the given boolean values.

[`base64decode`](#base64decode)

Decodes a string containing a base64 sequence.

[`base64encode`](#base64encode)

Encodes a string to a base64 sequence.
...
Returns the total number of bytes in the given buffer.

[`bytesslice`](#bytesslice)

Extracts a subslice from the given buffer.

[`can`](#can)

Tries to evaluate the expression given in its first argument.
...
Calculates a full host IP address within a given IP network address prefix.
...
Calculates many consecutive subnet addresses at once, rather than just a single subnet extension.

[`coalesce`](#coalesce)

Returns the first of the given arguments that isn't null, or raises an error if there are no non-null arguments.
...
Converts a value to a specified type constraint, using HCL's customdecode extension for type expression support.
...
Returns true if the two given values are equal, or false otherwise.
...
Returns true if and only if the second number is greater than the first.

[`greaterthanorequalto`](#greaterthanorequalto)

Returns true if and only if the second number is greater than or equal to the first.

[`hasindex`](#hasindex)

Returns true if if the given collection can be indexed with the given key without producing an error, or false otherwise.
...
Discards any fractional portion of the given number.
...
Returns true if and only if the second number is less than the first.

[`lessthanorequalto`](#lessthanorequalto)

Returns true if and only if the second number is less than or equal to the first.
...
Returns the value of the element with the given key from the given map, or returns the default value if there is no such element.
...
Divides the first given number by the second and then returns the remainder.
...
Applies the logical NOT operation to the given boolean value.

[`notequal`](#notequal)

Returns false if the two given values are equal, or true otherwise.

[`or`](#or)

Applies the logical OR operation to the given boolean values.
...
Replaces all non-alphanumeric characters with a underscore, leaving only characters that are valid for a Bake target name.

[`semvercmp`](#semvercmp)

Returns true if version satisfies a constraint.

[`sethaselement`](#sethaselement)

Returns true if the given set contains the given element, or false otherwise.
...
Variadic function that tries to evaluate all of is arguments in sequence until one succeeds, in which case it returns that result, or returns an error if none of them succeed.
...
Generates and returns a Type-4 UUID in the standard hexadecimal string format.

[`uuidv5`](#uuidv5)

Generates and returns a Type-5 UUID in the standard hexadecimal string format.
...
[Request changes](https://github.com/docker/docs/issues/new?template=doc_issue.yml\&location=https%3a%2f%2fdocs.docker.com%2fbuild%2fbake%2fstdlib%2f\&labels=status%2Ftriage)
