...
Converts a [duration](https://docs.docker.com/reference/compose-file/extension/#specifying-durations) into an integer (seconds).
...
Converts a string into a safe identifier (replaces non-lowercase characters with `-`).
...
Joins list elements into a single string with a separator.

`base64`

Encodes a string as base64 (used for Kubernetes secrets).

`map`

Maps values using `“value -> newValue”` syntax.

`indent`

Indents string content by N spaces.

`helmValue`

Outputs a Helm-style template value.
...
[Edit this page](https://github.com/docker/docs/edit/main/content/manuals/compose/bridge/customize.md)

[Request changes](https://github.com/docker/docs/issues/new?template=doc_issue.yml\&location=https%3a%2f%2fdocs.docker.com%2fcompose%2fbridge%2fcustomize%2f\&labels=status%2Ftriage)
