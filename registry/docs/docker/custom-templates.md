...
## [When to use custom templates](#when-to-use-custom-templates)
...
## [Building templates with Dockerfiles](#building-templates-with-dockerfiles)
...
### [The USER pattern](#the-user-pattern)
...
### [Using templates built from Dockerfiles](#using-templates-built-from-dockerfiles)
...
The default `--pull-template missing` policy finds the image in your local Docker daemon without pulling from a registry.
...
## [Template caching and pull policies](#template-caching-and-pull-policies)
...
- `--pull-template missing` (default)
  ...
  Use the local image if present, otherwise pull from the registry. Works for both locally-built images and registry images. On first use, registry images are pulled and cached; subsequent sandboxes reuse the cache.

- `--pull-template always`

  Always pull the latest image from the registry and update the host cache. The VM uses the refreshed cache. Slower than `missing` but guarantees the sandbox uses the latest version. Requires a registry image.

- `--pull-template never`

  Never use the host cache. The sandbox VM pulls the image directly from the registry on every startup. Requires a registry image.
...
## [Creating templates from existing sandboxes](#creating-templates-from-existing-sandboxes)
...
This saves the image to your local Docker daemon. The default pull policy finds the image locally:
...
## [Example: Node.js template](#example-nodejs-template)
...
## [Sharing with teams](#sharing-with-teams)
...
[Edit this page](https://github.com/docker/docs/edit/main/content/manuals/ai/sandboxes/templates.md)

[Request changes](https://github.com/docker/docs/issues/new?template=doc_issue.yml\&location=https%3a%2f%2fdocs.docker.com%2fai%2fsandboxes%2ftemplates%2f\&labels=status%2Ftriage)
