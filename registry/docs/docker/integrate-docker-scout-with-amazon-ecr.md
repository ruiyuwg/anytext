...
SNS topic for notifying Docker Scout when the AWS resources have been created.
...
Defines the topic for the initial setup notification.
...
Stores the credentials used by EventBridge to fire events to Scout.
...
Sets up the EventBridge connection to Docker Scout for sending ECR push and delete events.
...
EventBridge connection credentials to Scout.
...
Defines the rule to send ECR pushes and deletes to Scout.
...
Defines the rule to send ECR repository deletes to Scout.
...
Internal role to grant the event access to `ApiDestination`.
...
This role has access to `ScoutAPICredentials` for setting up the Docker Scout integration.
...
[Edit this page](https://github.com/docker/docs/edit/main/content/manuals/scout/integrations/registry/ecr.md)

[Request changes](https://github.com/docker/docs/issues/new?template=doc_issue.yml\&location=https%3a%2f%2fdocs.docker.com%2fscout%2fintegrations%2fregistry%2fecr%2f\&labels=status%2Ftriage)
