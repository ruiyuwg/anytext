...
Agent names for task delegation
...
Agent names for conversation handoff
...
Include working directory, OS, Git info
...
Maximum tool call loops (unlimited if not set)
...
Repetition penalty (-2.0 to 2.0, OpenAI only)
...
Topic penalty (-2.0 to 2.0, OpenAI only)
...
`chunking.respect_word_boundaries`
...
Document paths (supports glob patterns, respects `.gitignore`)

`tool`

object

Customize RAG tool name/description/instruction

`strategies`

\[]object

Retrieval strategies (see above for strategy-specific fields)

`results`

object

Post-processing (fusion, reranking, limits)
...
[Request changes](https://github.com/docker/docs/issues/new?template=doc_issue.yml\&location=https%3a%2f%2fdocs.docker.com%2fai%2fdocker-agent%2freference%2fconfig%2f\&labels=status%2Ftriage)
