...
Prevents developers from overriding settings

`true` (locked), `false` (default)

`mode`

Proxy configuration method

`system` (use system proxy), `manual` (custom)
...
URL (e.g., `"https://proxy.company.com:8080"`)
...
Proxy Auto-Configuration file URL

URL to PAC file

`transparentPorts`

Ports subject to proxy rules

Comma-separated ports or wildcard (`"*"`)
...
Route through HTTP proxy at specified host and port

`SOCKS5 host:port`

Route through SOCKS5 proxy at specified host and port

`DIRECT`

Allow direct connection without proxy
...
[Edit this page](https://github.com/docker/docs/edit/main/content/manuals/enterprise/security/hardened-desktop/air-gapped-containers.md)

[Request changes](https://github.com/docker/docs/issues/new?template=doc_issue.yml\&location=https%3a%2f%2fdocs.docker.com%2fenterprise%2fsecurity%2fhardened-desktop%2fair-gapped-containers%2f\&labels=status%2Ftriage)
