...
If present, causes the bind mount to be [mounted into the container as read-only](#use-a-read-only-bind-mount).

`bind-propagation`

If present, changes the [bind propagation](#configure-bind-propagation).
...
Configures SELinux labeling. See [Configure the SELinux label](#configure-the-selinux-label)

`rprivate` (default)

Sets bind propagation to `rprivate` for this mount. See [Configure bind propagation](#configure-bind-propagation).

`private`

Sets bind propagation to `private` for this mount. See [Configure bind propagation](#configure-bind-propagation).

`rshared`

Sets bind propagation to `rshared` for this mount. See [Configure bind propagation](#configure-bind-propagation).

`shared`

Sets bind propagation to `shared` for this mount. See [Configure bind propagation](#configure-bind-propagation).

`rslave`

Sets bind propagation to `rslave` for this mount. See [Configure bind propagation](#configure-bind-propagation).

`slave`

Sets bind propagation to `slave` for this mount. See [Configure bind propagation](#configure-bind-propagation).
...
[Edit this page](https://github.com/docker/docs/edit/main/content/manuals/engine/storage/bind-mounts.md)

[Request changes](https://github.com/docker/docs/issues/new?template=doc_issue.yml\&location=https%3a%2f%2fdocs.docker.com%2fengine%2fstorage%2fbind-mounts%2f\&labels=status%2Ftriage)
