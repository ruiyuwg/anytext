...
Full registry path, such as `dhi.io/my-image`.

`variant`

Image variant type: `runtime` or `dev`.
...
Target architectures, such as `linux/amd64` and `linux/arm64`.

`contents`

Package repositories and packages to install.
...
Defines the `/etc/os-release` contents inside the image.

`annotations`

OCI image annotations such as description and license.
...
Image flavor modifier, such as `compat` for compatibility images.
...
Users, groups, and the `run-as` user.
...
Directories, files, and symlinks to create.
...
Build stages with shell pipelines.

`contents.keyring`

Signing keys for third-party package repositories.

`contents.artifacts`

Pre-built OCI artifacts to include.

`contents.mappings`

Package URL (purl) mappings for SBOM accuracy.

`contents.files`

Source files fetched from Git URLs with checksums.
...
Each build stage has its own `contents` section. Include packages needed only during the build, such as `bash`.

`pipeline`

Contains named steps that run shell commands. Always start scripts with `set -eux -o pipefail`.

`outputs`

Copies results from the build stage into the final image. Setting `diff: true` copies only files that changed, keeping the image minimal.

`accounts`

Nginx uses a dedicated `nginx` user (UID 65532) instead of `nonroot`. The `www-data` group (GID 82) is also created for web server compatibility.

`musl-utils`

Required in both the main and build packages for Alpine-based Nginx images.
...
Full OCI reference with digest pin. Always use `@sha256:` for reproducibility.

`includes`

Glob patterns for files to extract from the artifact. Paths are resolved from the filesystem root; `opt/**` includes everything under the `/opt` path.

`excludes`

Glob patterns for files to skip. Useful for removing headers, docs, or unused binaries.

`uid`, `gid`

Ownership for extracted files.
...
[Edit this page](https://github.com/docker/docs/edit/main/content/manuals/dhi/how-to/build.md)

[Request changes](https://github.com/docker/docs/issues/new?template=doc_issue.yml\&location=https%3a%2f%2fdocs.docker.com%2fdhi%2fhow-to%2fbuild%2f\&labels=status%2Ftriage)
