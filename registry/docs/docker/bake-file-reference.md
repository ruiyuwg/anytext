...
Specify the frontend method to call for the target.
...
[`dockerfile-inline`](#targetdockerfile-inline)

String

Inline Dockerfile string
...
[`entitlements`](#targetentitlements)

List

Permissions that the build process requires to run

[`extra-hosts`](#targetextra-hosts)

List

Customs host-to-IP mapping

[`inherits`](#targetinherits)
...
Define a set of variables that forks a target into multiple targets.

[`name`](#targetname)

String

Override the target name when using a matrix.

[`no-cache-filter`](#targetno-cache-filter)

List

Disable build cache for specific stages

[`no-cache`](#targetno-cache)

Boolean

Disable build cache completely
...
Policies to validate build sources and metadata
...
`docker-image://alpine@sha256:0123456789`
...
Holds the main context when building using a remote Bake file.
...
[Request changes](https://github.com/docker/docs/issues/new?template=doc_issue.yml\&location=https%3a%2f%2fdocs.docker.com%2fbuild%2fbake%2freference%2f\&labels=status%2Ftriage)
