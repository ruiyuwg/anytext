...
* Ensure that you have owner or contributor access to the subscription to create a Data Box order.
* Review the [safety guidelines for your Data Box](data-box-safety.md).
* Ensure that you have an Azure Data Box device to move data to your location. Your host computer must
  * Run a [Supported operating system](data-box-system-requirements.md).
...
1. Select an existing subscription enabled for this service and choose transfer type as **Export to Azure**. Provide the **Source Azure region** where the data resides and **Destination country** for the data transfer.
...
1. Select **Add storage account and export type** and then **Select Export option**.
...
1. Create a new Azure Data Box resource in Azure portal.
1. Select an existing subscription enabled for this service and choose transfer type as **Export to Azure**. Provide the **Source Azure region** where the data resides and **Destination country** for the data transfer.
1. Select **Data Box**. The maximum usable capacity is 80 TB and you can create multiple orders for larger data sizes.
1. Select **Add storage account and export type** and then **Select Export option**.
1. Enter the order details and shipping information. If the service is available in your region, provide notification email addresses, review the summary, and then create the order.
:::zone-end
...
1. Cable the device as shown below.  
    ...
    1. Use the RJ-45 CAT 6 network cable to connect your host computer to the MGMT port on the device.
    1. Use the QFSP28 copper cable to connect at least one 100 Gbps (preferred over 10 or 1 Gbps) network interface, DATA 1 or DATA 2 for data.  
    1. Turn on the device. The power button is on the front panel of the device.
:::zone-end
...
1. Cable the device as shown below.  
    ...
    - Use the RJ-45 CAT 6 network cable to connect your host computer to the MGMT port on the device.
    - Use the SFP+ Twinax copper cable to connect at least one 10 Gbps (preferred over 1 Gbps) network interface, DATA 1, or DATA 2 for data.
    - Turn on the device. The power button is on the front panel of the device.
...
1. Assign a static IP address of 192.168.100.5 and subnet 255.255.255.0 to the Ethernet adapter on the computer you're using to connect to Data Box. Access the local web UI of the device at `https://192.168.100.10`. The connection could take up to 5 minutes after you turned on the device.
...
1. By default, the network settings for the 100-Gbps data interface (or 1 Gbps) are configured as DHCP. If needed, you can configure this interface as static and provide an IP address.
:::zone-end
...
1. Connect to the device shares and begin copying data to the host computer.
...
1. Download and print the return shipping label when preparation is complete.
1. Turn off the device from the local web UI. Remove the cables from the device.
1. Spool and securely place the power cord that was provided with device in the back of the device.
1. Pack the device in the original box and secure firmly. Paste the return shipping label on the box and remove any previous labels and barcode stickers from the external packaging.
1. Follow local return shipping instructions and ship the device back to Microsoft. 
...
1. Turn off the device from the local web UI. Remove the cables from the device.
1. Spool and securely place the power cord that was provided with device in the back of the device.
1. The return shipping label should be visible on the E-ink display. If the E-ink display isn't displaying the label, go to **Overview** > **Download shipping label** in the Azure portal. Download the shipping label and affix on the device.
1. Schedule a pickup with your regional carrier if returning the device.
1. Once the Data Box is picked up and scanned by your carrier, the order status in the portal updates to **Picked up**. A tracking ID is also displayed.
:::zone-end
