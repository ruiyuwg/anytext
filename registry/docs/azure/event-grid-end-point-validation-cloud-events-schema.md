---
title: Endpoint Validation Using CloudEvents v1.0 Schema
description: Learn how to validate Webhook endpoints using the CloudEvents v1.0 schema and ensure secure event delivery with abuse protection mechanisms.
...
# Endpoint validation by using CloudEvents schema
...
## Endpoint validation by using CloudEvents v1.0
CloudEvents v1.0 implements its own abuse protection semantics by using the **HTTP OPTIONS** method. When you use the CloudEvents schema for output, Event Grid uses the CloudEvents v1.0 abuse protection in place of the Event Grid validation event mechanism.
...
The following validation handshake realizes reaching the delivery agreement. The handshake can either be executed immediately at registration time
...
The validation request uses the **HTTP OPTIONS** method. The request goes to the exact resource target URI that you're registering. By using the validation request, the sender asks the target for permission to send notifications, and it can declare a desired request rate (requests per minute). The delivery target responds with a permission statement and the permitted request rate. Here's a couple of the header fields for inclusion in the validation request.
...
Domain Name System (DNS) expression that identifies the sending system, such as `eventemitter.example.com`. The value summarily identifies all sender
