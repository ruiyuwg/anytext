...
To deploy your program on an IoT Edge device, you must first containerize it and run it using a Docker-compatible engine. IoT Edge uses [Moby](https://github.com/moby/moby), the open-source project behind Docker, as its Docker-compatible engine. You can pass the same parameters that you're used to with Docker to your IoT Edge modules. For more information, see [How to configure container create options for IoT Edge modules](how-to-use-create-options.md).

## Using the IoT Edge hub

An IoT Edge hub provides two main functionalities: a proxy to IoT Hub and local communications.
...
* Process your message using this callback function and (optionally) send new messages in your module endpoint queue.

> [!NOTE]
> For more information about declaring a route, see [Learn how to deploy modules and establish routes in IoT Edge](module-composition.md#declare-routes).

#### Twins

*Twins* are one of the primitives provided by IoT Hub. Twins are JSON documents that store state information including metadata, configuration, and conditions. Each module or device has its own twin.

* To get a module twin using the [Azure IoT SDK](https://github.com/Azure/azure-iot-sdks), call the `ModuleClient.getTwin` method.
* To receive a module twin patch using the Azure IoT SDK, implement a callback function and register it by using the `ModuleClient.moduleTwinCallback` method so that your callback function is triggered each time a twin patch comes in.

#### Receive direct methods

To receive a direct method using the [Azure IoT SDK](https://github.com/Azure/azure-iot-sdks), implement a callback function and register it by using the `ModuleClient.methodCallback` method so that your callback function is triggered each time that a direct method comes in.
...
> [!NOTE]
> For cross-platform compilation, like compiling an ARM32 IoT Edge module on an AMD64 development machine, you must configure the development machine to compile code on target device architecture matching the IoT Edge module. For more information about target device architectures, see [Tutorial: Develop Azure IoT Edge modules using Visual Studio Code](tutorial-develop-for-linux.md).

### Windows

Microsoft no longer supports Windows containers. [IoT Edge for Linux on Windows](iot-edge-for-linux-on-windows.md) is the recommended way to run IoT Edge on Windows devices.

## Module security

Develop your modules with security in mind. To learn more about securing your modules, see [Docker Engine security](https://docs.docker.com/engine/security/).
...
In the config file on an IoT Edge device, a parameter called `allow_elevated_docker_permissions` controls elevated Docker permissions. When set to **true**, this flag grants the `--privileged` flag and any other capabilities that you define in the `CapAdd` field of the Docker HostConfig in the [container create options](how-to-use-create-options.md). 
...
The Docker capabilities **CAP_CHOWN** and **CAP_SETUID** are disabled by default. These capabilities can write to secure files on the host device and potentially gain root access.

If you need these capabilities, manually re-enable them by using `CapADD` in the container create options.
