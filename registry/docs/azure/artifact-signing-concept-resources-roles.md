---
title: Artifact Signing resources and roles
...
Artifact Signing is an Azure-native resource that offers full support for common Azure concepts, such as resources. Like with any other Azure resource, Artifact Signing has its own set of resources and roles that are designed to simplify management of the service.

This article introduces you to the resources and roles that are specific to Artifact Signing.

## Artifact Signing resource types

Artifact Signing has the following resource types:

- **Artifact Signing account**: An account is a logical container of all the resources you need to complete signing and manage access controls to sensitive resources.
...
In the following example structure, an Azure subscription has a resource group. Under the resource group, you can have one or many Artifact Signing account resources with one or many identity validations and certificate profiles.
...
For more information about Public Trust and Private Trust, see [Artifact Signing trust models](./concept-trust-models.md).

### Certificate profiles

Artifact Signing provides five total certificate profile types that all subscribers can use with the aligned and completed identity validation resources. These five certificate profiles are aligned to Public Trust or Private Trust identity validations as follows:
...
Role-based access control (RBAC) is a cornerstone concept for all Azure resources. Artifact Signing adds two custom roles to meet subscriber needs for creating an identity validation (the Artifact Signing Identity Verifier role) and signing with certificate profiles (the Artifact Signing Certificate Profile Signer role). These custom roles explicitly must be assigned to perform those two critical functions in using Artifact Signing. The following table contains a complete list of roles that Artifact Signing supports and their capabilities, including all standard Azure roles.
...
|---------------|---------------|-----------------|-----------------|-----------------|-----------------|-----------------|
|Artifact Signing Identity Verifier<sub>1</sub>||||||X|
|Artifact Signing Certificate Profile Signer<sub>2</sub>|||X|X|||
|Owner|X|X|||X||
|Contributor|X|X|||||
|Reader|X||||||
|User Access Admin|||||X||
...
<sub>2</sub> Required to successfully sign by using Artifact Signing.
