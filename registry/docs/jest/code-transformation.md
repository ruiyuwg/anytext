...
Jest supports this via the [`transform`](/docs/configuration#transform-objectstring-pathtotransformer--pathtotransformer-object) configuration option.
...
During the development of a transformer it can be useful to run Jest with `--no-cache` to frequently [delete cache](/docs/troubleshooting#caching-issues).
