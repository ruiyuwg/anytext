...
-   [Asymmetric Matchers](#asymmetric-matchers)
    ...
    -   [`expect.arrayOf(value)`](#expectarrayofvalue)
    -   [`expect.not.arrayOf(value)`](#expectnotarrayofvalue)
...
Since you are still testing promises, the test is still asynchronous. Hence, you will need to [tell Jest to wait](/docs/asynchronous#promises) by returning the unwrapped assertion.
...
This ensures that a value matches the most recent snapshot. Check out [the Snapshot Testing guide](/docs/snapshot-testing) for more information.
...
Check out the section on [Inline Snapshots](/docs/snapshot-testing#inline-snapshots) for more info.
...
### `expect.arrayOf(value)`[​](#expectarrayofvalue "Direct link to expectarrayofvalue")
...
```
test('all elements in array are strings', () => {  expect(['apple', 'banana', 'cherry']).toEqual(    expect.arrayOf(expect.any(String)),  );});
```

This matcher is particularly useful for validating arrays containing complex structures:

```
test('array of objects with specific properties', () => {  expect([    {id: 1, name: 'Alice'},    {id: 2, name: 'Bob'},  ]).toEqual(    expect.arrayOf(      expect.objectContaining({        id: expect.any(Number),        name: expect.any(String),      }),    ),  );});
```

### `expect.not.arrayOf(value)`[​](#expectnotarrayofvalue "Direct link to expectnotarrayofvalue")
...
```
test('not all elements in array are strings', () => {  expect(['apple', 123, 'cherry']).toEqual(    expect.not.arrayOf(expect.any(String)),  );});
```
...
```
import {expect} from '@jest/globals';import {Volume} from './Volume.js';function areVolumesEqual(a: unknown, b: unknown): boolean | undefined {  const isAVolume = a instanceof Volume;  const isBVolume = b instanceof Volume;  if (isAVolume && isBVolume) {    return a.equals(b);  } else if (isAVolume === isBVolume) {    return undefined;  } else {    return false;  }}expect.addEqualityTesters([areVolumesEqual]);
```

\_\_tests\_\_/Volume.test.ts
...
See [configuring Jest](/docs/configuration#snapshotserializers-arraystring) for more information.
...
Instead of importing `toBeWithinRange` module to the test file, you can enable the matcher for all tests by moving the `expect.extend` call to a [`setupFilesAfterEnv`](/docs/configuration#setupfilesafterenv-array) script:
