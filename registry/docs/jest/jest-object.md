...
-   [Mock Modules](#mock-modules)
    ...
    -   [`jest.onGenerateMock(cb)`](#jestongeneratemockcb)
...
Automatic mocking should be enabled via [`automock`](/docs/configuration#automock-boolean) configuration option for this method to have any effect. Also see documentation of the configuration option for more details.
...
```
import type {Config} from 'jest';const config: Config = {  automock: true,};export default config;
```
...
For more details on automatic mocking see documentation of [`automock`](/docs/configuration#automock-boolean) configuration option.
...
This is useful when you want to create a [manual mock](/docs/manual-mocks) that extends the automatic mock's behavior:
...
```
export const utils = {  authorize: () => {    return 'token';  },  isAuthorized: (secret: string) => secret === 'wizard',};
```

\_\_tests\_\_/createMockFromModule.test.ts

```
const {utils} =  jest.createMockFromModule<typeof import('../utils')>('../utils');utils.isAuthorized = jest.fn((secret: string) => secret === 'not wizard');test('implementation created by jest.createMockFromModule', () => {  expect(jest.isMockFunction(utils.authorize)).toBe(true);  expect(utils.isAuthorized('not wizard')).toBe(true);});
```
...
Creates a new [mock function](/docs/mock-function-api). The new function has no formal parameters and when called will return `undefined`. This functionality also applies to `async` functions.
...
```
// The optional type argument provides typings for the module factoryjest.mock<typeof import('../moduleName')>('../moduleName', () => {  return jest.fn(() => 42);});// This runs the function specified as second argument to `jest.mock`.const moduleName = require('../moduleName');moduleName(); // Will return '42';
```
...
Importing a module in a setup file (as specified by [`setupFilesAfterEnv`](/docs/configuration#setupfilesafterenv-array)) will prevent mocking for the module in question, as well as all the modules that it imports.
...
Writing tests in TypeScript? Use the [`jest.Mocked`](/docs/mock-function-api#jestmockedsource) utility type or the [`jest.mocked()`](/docs/mock-function-api#jestmockedsource-options) helper method to have your mocked modules typed.

### `jest.Mocked<Source>`[​](#jestmockedsource "Direct link to jestmockedsource")

See [TypeScript Usage](/docs/mock-function-api#jestmockedsource) chapter of Mock Functions page for documentation.

### `jest.mocked(source, options?)`[​](#jestmockedsource-options "Direct link to jestmockedsource-options")

See [TypeScript Usage](/docs/mock-function-api#jestmockedsource-options) chapter of Mock Functions page for documentation.
...
```
beforeEach(() => {  jest.resetModules();});test('moduleName 1', () => {  // The optional type argument provides typings for the module factory  jest.doMock<typeof import('../moduleName')>('../moduleName', () => {    return jest.fn(() => 1);  });  const moduleName = require('../moduleName');  expect(moduleName()).toBe(1);});test('moduleName 2', () => {  jest.doMock<typeof import('../moduleName')>('../moduleName', () => {    return jest.fn(() => 2);  });  const moduleName = require('../moduleName');  expect(moduleName()).toBe(2);});
```
...
On occasion, there are times where the automatically generated mock the module system would normally provide you isn't adequate enough for your testing needs. Normally under those circumstances you should write a [manual mock](/docs/manual-mocks) that is more adequate for the module in question. However, on extremely rare occasions, even a manual mock isn't suitable for your purposes and you need to build the mock yourself inside your test.
...
```
jest.mock('../myModule', () => {  // Require the original module to not be mocked...  const originalModule =    jest.requireActual<typeof import('../myModule')>('../myModule');  return {    __esModule: true, // Use it when dealing with esModules    ...originalModule,    getRandom: jest.fn(() => 10),  };});const getRandom = require('../myModule').getRandom;getRandom(); // Always returns 10
```
...
### `jest.onGenerateMock(cb)`[​](#jestongeneratemockcb "Direct link to jestongeneratemockcb")
...
```
jest.onGenerateMock((modulePath, moduleMock) => {  // Inspect the module name and decide how to transform the mock  if (modulePath.includes('Database')) {    // For demonstration, let's replace a method with our own custom mock    moduleMock.connect = jest.fn().mockImplementation(() => {      console.log('Connected to mock DB');    });  }  // Return the (potentially modified) mock  return moduleMock;});// Apply mock for modulejest.mock('./Database');// Later in your testsimport Database from './Database';// The `Database` mock now has any transformations applied by our callback
```
...
Returns a new, unused [mock function](/docs/mock-function-api). Optionally takes a mock implementation.
...
See the [Mock Functions](/docs/mock-function-api#jestfnimplementation) page for details on TypeScript usage.
...
Replace `object[propertyKey]` with a `value`. The property must already exist on the object. The same property might be replaced multiple times. Returns a Jest [replaced property](/docs/mock-function-api#replaced-properties).
...
All properties replaced with `jest.replaceProperty` could be restored to the original value by calling [jest.restoreAllMocks](#jestrestoreallmocks) on [afterEach](/docs/api#aftereachfn-timeout) method.
...
Creates a mock function similar to `jest.fn` but also tracks calls to `object[methodName]`. Returns a Jest [mock function](/docs/mock-function-api).
...
Since `jest.spyOn` is a mock, you could restore the initial state by calling [`jest.restoreAllMocks`](#jestrestoreallmocks) in the body of the callback passed to the [afterEach](/docs/api#aftereachfn-timeout) hook.
...
See [TypeScript Usage](/docs/mock-function-api#replacedpropertyreplacevaluevalue) chapter of Mock Functions page for documentation.

### `jest.Spied<Source>`[​](#jestspiedsource "Direct link to jestspiedsource")

See [TypeScript Usage](/docs/mock-function-api#jestspiedsource) chapter of Mock Functions page for documentation.

### `jest.clearAllMocks()`[​](#jestclearallmocks "Direct link to jestclearallmocks")

Clears the `mock.calls`, `mock.instances`, `mock.contexts` and `mock.results` properties of all mocks. Equivalent to calling [`.mockClear()`](/docs/mock-function-api#mockfnmockclear) on every mocked function.

Returns the `jest` object for chaining.

### `jest.resetAllMocks()`[​](#jestresetallmocks "Direct link to jestresetallmocks")

Resets the state of all mocks. Equivalent to calling [`.mockReset()`](/docs/mock-function-api#mockfnmockreset) on every mocked function.

Returns the `jest` object for chaining.

### `jest.restoreAllMocks()`[​](#jestrestoreallmocks "Direct link to jestrestoreallmocks")

Restores all mocks and replaced properties back to their original value. Equivalent to calling [`.mockRestore()`](/docs/mock-function-api#mockfnmockrestore) on every mocked function and [`.restore()`](/docs/mock-function-api#replacedpropertyrestore) on every replaced property. Beware that `jest.restoreAllMocks()` only works for mocks created with [`jest.spyOn()`](#jestspyonobject-methodname) and properties replaced with [`jest.replaceProperty()`](#jestreplacepropertyobject-propertykey-value); other mocks will require you to manually restore them.
...
Legacy fake timers will swap out `setImmediate()`, `clearImmediate()`, `setInterval()`, `clearInterval()`, `setTimeout()`, `clearTimeout()` with Jest [mock functions](/docs/mock-function-api). In Node environment `process.nextTick()` and in JSDOM environment `requestAnimationFrame()`, `cancelAnimationFrame()` will be also replaced.
...
This is often useful for synchronously executing setTimeouts during a test in order to synchronously assert about some behavior that would only happen after the `setTimeout()` or `setInterval()` callbacks executed. See the [Timer mocks](/docs/timer-mocks) doc for more information.
...
Use the [`--showSeed`](/docs/cli#--showseed) flag to print the seed in the test report summary. To manually set the value of the seed use [`--seed=<num>`](/docs/cli#--seednum) CLI argument.
...
To set timeout intervals on different tests in the same file, use the [`timeout` option on each individual test](/docs/api#testname-fn-timeout).

If you want to set the timeout for all test files, use [`testTimeout`](/docs/configuration#testtimeout-number) configuration option.
