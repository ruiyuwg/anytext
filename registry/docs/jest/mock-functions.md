...
There are two ways to mock functions: Either by creating a mock function to use in test code, or writing a [`manual mock`](/docs/manual-mocks) to override a module dependency.
...
You can optionally provide a name for your mock functions, which will be displayed instead of `'jest.fn()'` in the test error output. Use [`.mockName()`](/docs/mock-function-api#mockfnmocknamename) if you want to be able to quickly identify the mock function reporting an error in your test output.
...
For a complete list of matchers, check out the [reference docs](/docs/expect).
