...
By default, a user can only request a magic link once every auth.rate\_limits.magic\_link.period and they expire after auth.rate\_limits.magic\_link.validity.
...
By default, a user can only request an OTP once every auth.rate\_limits.otp.period and they expire after auth.rate\_limits.otp.validity. This is configurable via `Auth > Providers > Email > Email OTP Expiration`. An expiry duration of more than 86400 seconds (one day) is disallowed to guard against brute force attacks. The longer an OTP remains valid, the more time an attacker has to attempt brute force attacks. If the OTP is valid for several days, an attacker might have more opportunities to guess the correct OTP through repeated attempts.
...
```dart
Future signInWithEmailOtp() async {
  final AuthResponse res = await supabase.auth.signInWithOtp(email: 'valid.email@supabase.io');
}
```
