...
Outside of runtime errors, both HTTP Hooks and Postgres Hooks return timeout errors. Postgres Hooks have auth.hook\_timeouts.postgres\_hooks seconds to complete processing while HTTP Hooks should complete in auth.hook\_timeouts.http\_hooks seconds. Both HTTP Hooks and Postgres Hooks are run in a transaction do limit the duration of execution to avoid delays in authentication process.
