...
- Global CSS should not be imported into this file. Instead, use the [Root Layout](/router/basics/layout#root-layout). Expo Router traverses the dependency graph starting from the root layout, so importing CSS elsewhere can cause unexpected loading order where **node\_modules** CSS takes precedence over your custom styles.
