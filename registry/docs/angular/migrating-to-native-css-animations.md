...
- arrow\_back Legacy Animations
  - [Overview](/guide/legacy-animations)
  - [Transition and Triggers](/guide/legacy-animations/transition-and-triggers)
  - [Complex Sequences](/guide/legacy-animations/complex-sequences)
  - [Reusable Animations](/guide/legacy-animations/reusable-animations)
  - [Migrating to Native CSS Animations](/guide/animations/migration)
...
Just like with the animations package, you can create reusable animations that can be shared across your application. The animations package version of this had you using the [`animation()`](/api/animations/animation) function in a shared typescript file. The native CSS version of this is similar, but lives in a shared CSS file.

#### [With Animations Package](#with-animations-package)
...
#### [With Native CSS](#with-native-css)
...
The animations package allowed you to define various states using the [](api/animations/state)[`state()`](/api/animations/state) function within a component. Examples might be an `open` or `closed` state containing the styles for each respective state within the definition. For example:
...
The animations package [`animate()`](/api/animations/animate) function allows for providing timing, like duration, delays and easing. This can be done natively with CSS using several css properties or shorthand properties.
...
### [Predefined State and wildcard matching](#predefined-state-and-wildcard-matching)
...
### [Automatic Property Calculation with Wildcards](#automatic-property-calculation-with-wildcards)
...
```
.container {  display: block;  overflow: hidden;}.container .content {  padding: 20px;  margin-top: 1em;  font-weight: bold;  font-size: 20px;  background: #3b82f6;  color: #ebebeb;}
```
...
```
import {Component} from '@angular/core';import {trigger, transition, animate, style} from '@angular/animations';@Component({  selector: 'app-insert-remove',  animations: [    trigger('myInsertRemoveTrigger', [      transition(':enter', [style({opacity: 0}), animate('200ms', style({opacity: 1}))]),      transition(':leave', [animate('200ms', style({opacity: 0}))]),    ]),  ],  templateUrl: 'insert-remove.html',  styleUrls: ['insert-remove.css'],})export class InsertRemove {  isShown = false;  toggle() {    this.isShown = !this.isShown;  }}
```
...
### [Parent / Child Animations](#parent---child-animations)
...
In the animations package, you could target specific elements by using the [`query()`](/api/animations/query) function to find specific elements by a CSS class name, similar to [`document.querySelector()`](https://developer.mozilla.org/en-US/docs/Web/API/Element/querySelector). This is unnecessary in a native CSS animation world. Instead, you can use your CSS selectors to target sub-classes and apply any desired `transform` or `animation`.
...
The [`stagger()`](/api/animations/stagger) function allowed you to delay the animation of each item in a list of items by a specified time to create a cascade effect. You can replicate this behavior in native CSS by utilizing `animation-delay` or `transition-delay`. Here is an example of what that CSS might look like.
...
```
import {Component, signal} from '@angular/core';import {trigger, transition, animate, query, style} from '@angular/animations';@Component({  selector: 'app-reorder',  templateUrl: './reorder.html',  styleUrls: ['reorder.css'],  animations: [    trigger('itemAnimation', [      transition(':enter', [        style({opacity: 0, transform: 'translateX(-10px)'}),        animate('300ms', style({opacity: 1, transform: 'translateX(none)'})),      ]),      transition(':leave', [        style({opacity: 1, transform: 'translateX(none)'}),        animate('300ms', style({opacity: 0, transform: 'translateX(-10px)'})),      ]),    ]),  ],})export class Reorder {  show = signal(true);  items = ['stuff', 'things', 'cheese', 'paper', 'scissors', 'rock'];  randomize() {    const randItems = [...this.items];    const newItems = [];    for (let i of this.items) {      const max: number = this.items.length - newItems.length;      const randNum = Math.floor(Math.random() * max);      newItems.push(...randItems.splice(randNum, 1));    }    this.items = newItems;  }}
```
...
The [`AnimationPlayer`](/api/animations/AnimationPlayer) class allows access to an animation to do more advanced things like pause, play, restart, and finish an animation through code. All of these things can be handled natively as well.
