...
1. ### [Generate the directive](#generate-the-directive)
...
2. ### [Make the directive structural](#make-the-directive-structural)

   Import [`TemplateRef`](/api/core/TemplateRef), and [`ViewContainerRef`](/api/core/ViewContainerRef). Inject [`TemplateRef`](/api/core/TemplateRef) and [`ViewContainerRef`](/api/core/ViewContainerRef) in the directive as private properties.
...
3. ### [Add the 'selectFrom' input](#add-the-selectfrom-input)

   Add a `selectFrom` [`input()`](/api/core/input) property.

   ```
   export class SelectDirective {  // ...  selectFrom = input.required();}
   ```

4. ### [Add the business logic](#add-the-business-logic)
...
### [How Angular translates shorthand](#how-angular-translates-shorthand)
...
`[prefixKey]="expression"` (The `prefix` is added to the `key`)

`let local`

`let-local="export"`

### [Shorthand examples](#shorthand-examples)
...
### [Type narrowing with template guards](#type-narrowing-with-template-guards)
...
```
// This directive only renders its template if the actor is a user.// You want to assert that within the template, the type of the `actor`// expression is narrowed to `User`.@Directive(...)class ActorIsUser {  actor = input<User | Robot>();  static ngTemplateGuard_actor(dir: ActorIsUser, expr: User | Robot): expr is User {    // The return statement is unnecessary in practice, but included to    // prevent TypeScript errors.    return true;  }}
```
...
```
@Directive(...)class CustomIf {  condition = input.required<boolean>();  static ngTemplateGuard_condition: 'binding';}
```
...
### [Typing the directive's context](#typing-the-directives-context)
