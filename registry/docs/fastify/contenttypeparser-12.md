...
Fastify automatically adds the parsed request payload to the [Fastify request](/docs/v5.6.x/Reference/Request/.md) object, accessible via `request.body`.
...
The request body can be parsed in two ways. First, add a custom content type parser and handle the request stream. Or second, use the `parseAs` option in the `addContentTypeParser` API, specifying `'string'` or `'buffer'`. Fastify will handle the stream, check the [maximum size](/docs/v5.6.x/Reference/Server/.md#factory-body-limit) of the body, and the content length. If the limit is exceeded, the custom parser will not be invoked.
...
- `bodyLimit` (number): The maximum payload size, in bytes, that the custom parser will accept. Defaults to the global body limit passed to the [`Fastify factory function`](/docs/v5.6.x/Reference/Server/.md#bodylimit).
