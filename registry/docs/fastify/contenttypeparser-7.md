...
The default charset is `utf-8`. If you need to support different content types, you can use the `addContentTypeParser` API. *The default JSON and/or plain text parser can be changed or removed.*
...
Fastify automatically adds the parsed request payload to the [Fastify request](/docs/v5.1.x/Reference/Request/.md) object which you can access with `request.body`.

Note that for `GET` and `HEAD` requests the payload is never parsed. For `OPTIONS` and `DELETE` requests the payload is only parsed if the content type is given in the content-type header. If it is not given, the [catch-all](#catch-all) parser is not executed as with `POST`, `PUT` and `PATCH`, but the payload is simply not parsed.

> ## ⚠ Security Notice[​](#--security-notice "Direct link to ⚠  Security Notice")
>
> When using with RegExp to detect `Content-Type`, you should beware of how to properly detect the `Content-Type`. For example, if you need `application/*`, you should use `/^application\/([\w-]+);?/` to match the [essence MIME type](https://mimesniff.spec.whatwg.org/#mime-type-miscellaneous) only.
...
- `bodyLimit` (number): The maximum payload size, in bytes, that the custom parser will accept. Defaults to the global body limit passed to the [`Fastify factory function`](/docs/v5.1.x/Reference/Server/.md#bodylimit).
