...
Fastify allows the user to extend its functionalities with plugins. A plugin can be a set of routes, a server [decorator](/docs/v5.0.x/Reference/Decorators/.md), or whatever. The API that you will need to use one or more plugins, is `register`.
...
You may have already seen in the [Getting Started](/docs/v5.0.x/Guides/Getting-Started/.md#your-first-plugin) guide how easy it is to use this API:
...
- [`logLevel`](/docs/v5.0.x/Reference/Routes/.md#custom-log-level)
- [`logSerializers`](/docs/v5.0.x/Reference/Routes/.md#custom-log-serializer)
...
If you pass an option with the key `prefix` with a `string` value, Fastify will use it to prefix all the routes inside the register, for more info check [here](/docs/v5.0.x/Reference/Routes/.md#route-prefixing).

Be aware that if you wrap your routes with [`fastify-plugin`](https://github.com/fastify/fastify-plugin), this option will not work (there is a [workaround](/docs/v5.0.x/Reference/Routes/.md#fastify-plugin) available).
...
Sometimes, you will need to know when the server is about to close, for example, because you must close a connection to a database. To know when this is going to happen, you can use the [`'onClose'`](/docs/v5.0.x/Reference/Hooks/.md#on-close) hook.
...
If you are using `register` only for extending the functionality of the server with [`decorate`](/docs/v5.0.x/Reference/Decorators/.md), it is your responsibility to tell Fastify not to create a new scope. Otherwise, your changes will not be accessible by the user in the upper scope.
