...
Fastify allows the user to extend its functionalities with plugins. A plugin can be a set of routes, a server [decorator](/docs/v3.29.x/Reference/Decorators/.md), or whatever. The API that you will need to use one or more plugins, is `register`.
...
You already see in the [getting started](/docs/v3.29.x/Guides/Getting-Started/.md#your-first-plugin) section how using this API is pretty straightforward.
...
- [`logLevel`](/docs/v3.29.x/Reference/Routes/.md#custom-log-level)
- [`logSerializers`](/docs/v3.29.x/Reference/Routes/.md#custom-log-serializer)
...
If you pass an option with the key `prefix` with a `string` value, Fastify will use it to prefix all the routes inside the register, for more info check [here](/docs/v3.29.x/Reference/Routes/.md#route-prefixing).
...
*async/await* is supported by `after`, `ready` and `listen`, as well as `fastify` being a [Thenable](https://promisesaplus.com/).
...
Sometimes, you will need to know when the server is about to close, for example, because you must close a connection to a database. To know when this is going to happen, you can use the [`'onClose'`](/docs/v3.29.x/Reference/Hooks/.md#on-close) hook.
...
If you are using `register` only for extending the functionality of the server with [`decorate`](/docs/v3.29.x/Reference/Decorators/.md), it is your responsibility to tell Fastify not to create a new scope. Otherwise, your changes will not be accessible by the user in the upper scope.
