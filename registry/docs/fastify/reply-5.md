...
- `.getResponseTime()` - Deprecated, returns the amount of time passed since the request was received by Fastify.
- `.context` - Deprecated, access the [Request's context](/docs/v4.29.x/Reference/Request/.md) property.
...
Note that unless this function is called in the [`onResponse` hook](/docs/v4.29.x/Reference/Hooks/.md#onresponse) it will always return `0`.
...
The Fastify server instance, scoped to the current [encapsulation context](/docs/v4.29.x/Reference/Encapsulation/.md).
...
Invokes the custom not found handler. Note that it will only call `preHandler` hook specified in [`setNotFoundHandler`](/docs/v4.29.x/Reference/Server/.md#set-not-found-handler).
...
*Note: This method is deprecated and will be removed in `fastify@5`. Use the [.elapsedTime](#elapsedtime) property instead.*
...
This is the [`http.ServerResponse`](https://nodejs.org/dist/latest-v14.x/docs/api/http.html#http_class_http_serverresponse) from Node core. Whilst you are using the Fastify `Reply` object, the use of `Reply.raw` functions is at your own risk as you are skipping all the Fastify logic of handling the HTTP response. e.g.:
...
If you want to customize error handling, check out [`setErrorHandler`](/docs/v4.29.x/Reference/Server/.md#seterrorhandler) API.
...
Beware that calling `reply.send(error)` in your custom error handler will send the error to the default error handler. Check out the [Reply Lifecycle](/docs/v4.29.x/Reference/Lifecycle/.md#reply-lifecycle) for more information.

The not found errors generated by the router will use the [`setNotFoundHandler`](/docs/v4.29.x/Reference/Server/.md#setnotfoundhandler)
...
The type of the sent payload (after serialization and going through any [`onSend` hooks](/docs/v4.29.x/Reference/Hooks/.md#onsend)) must be one of the following types, otherwise, an error will be thrown:
...
If you want to know more please review [Routes#async-await](/docs/v4.29.x/Reference/Routes/.md#async-await).
