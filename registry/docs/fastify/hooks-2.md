...
## Request/Response Hooks[​](#requestresponse-hooks "Direct link to Request/Response Hooks")
...
#### The `onSend` Hook[​](#the-onsend-hook "Direct link to the-onsend-hook")
...
\*\*'onClose'\*\*Triggered when `fastify.close()` is invoked to stop the server. It is useful when [plugins](https://github.com/fastify/fastify/blob/master/docs/Plugins.md) need a "shutdown" event, such as a connection to a database.The first argument is the Fastify instance, the second one the `done` callback.
...
### beforeHandler[​](#beforehandler "Direct link to beforeHandler")

Despite the name, `beforeHandler` is not a standard hook like `preHandler`, but is a function that your register right in the route option that will be executed only in the specified route. Can be useful if you need to handle the authentication at route level instead of at hook level (`preHandler` for example), it could also be an array of functions.**`beforeHandler` is executed always after the `preHandler` hook.**
