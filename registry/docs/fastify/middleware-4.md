...
You can also use [`@fastify/middie`](https://github.com/fastify/middie), which provides support for simple Express-style middleware but with improved performance:
...
Remember that middleware can be encapsulated; this means that you can decide where your middleware should run by using `register` as explained in the [plugins guide](/docs/v4.29.x/Guides/Plugins-Guide/.md).

Fastify middleware does not expose the `send` method or other methods specific to the Fastify [Reply](/docs/v4.29.x/Reference/Reply/.md#reply) instance. This is because Fastify wraps the incoming `req` and `res` Node instances using the [Request](/docs/v4.29.x/Reference/Request/.md#request) and [Reply](/docs/v4.29.x/Reference/Reply/.md#reply) objects internally, but this is done after the middleware phase. If you need to create middleware, you have to use the Node `req` and `res` instances. Otherwise, you can use the `preHandler` hook that already has the [Request](/docs/v4.29.x/Reference/Request/.md#request) and [Reply](/docs/v4.29.x/Reference/Reply/.md#reply) Fastify instances. For more information, see [Hooks](/docs/v4.29.x/Reference/Hooks/.md#hooks).
