...
The Fastify server instance, scoped to the current [encapsulation context](/docs/v5.5.x/Reference/Encapsulation/.md).
...
Invokes the custom not found handler. Note that it will only call `preHandler` hook specified in [`setNotFoundHandler`](/docs/v5.5.x/Reference/Server/.md#set-not-found-handler).
...
If you want to customize error handling, check out [`setErrorHandler`](/docs/v5.5.x/Reference/Server/.md#seterrorhandler) API.
...
Beware that calling `reply.send(error)` in your custom error handler will send the error to the default error handler. Check out the [Reply Lifecycle](/docs/v5.5.x/Reference/Lifecycle/.md#reply-lifecycle) for more information.

The not found errors generated by the router will use the [`setNotFoundHandler`](/docs/v5.5.x/Reference/Server/.md#setnotfoundhandler)
...
The type of the sent payload (after serialization and going through any [`onSend` hooks](/docs/v5.5.x/Reference/Hooks/.md#onsend)) must be one of the following types, otherwise, an error will be thrown:
...
If you want to know more please review [Routes#async-await](/docs/v5.5.x/Reference/Routes/.md#async-await).
