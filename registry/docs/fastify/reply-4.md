...
- `.context` - Access the [Request's context](/docs/v3.29.x/Reference/Request/.md) property.
...
The Fastify server instance, scoped to the current [encapsulation context](/docs/v3.29.x/Reference/Encapsulation/.md).
...
Invokes the custom not found handler. Note that it will only call `preHandler` hook specified in [`setNotFoundHandler`](/docs/v3.29.x/Reference/Server/.md#set-not-found-handler).
...
Note that unless this function is called in the [`onResponse` hook](/docs/v3.29.x/Reference/Hooks/.md#onresponse) it will always return `0`.
...
If you want to customize error handling, check out [`setErrorHandler`](/docs/v3.29.x/Reference/Server/.md#seterrorhandler) API.
...
The not found errors generated by the router will use the [`setNotFoundHandler`](/docs/v3.29.x/Reference/Server/.md#setnotfoundhandler)
...
The type of the sent payload (after serialization and going through any [`onSend` hooks](/docs/v3.29.x/Reference/Hooks/.md#onsend)) must be one of the following types, otherwise, an error will be thrown:
...
If you want to know more please review [Routes#async-await](/docs/v3.29.x/Reference/Routes/.md#async-await).
