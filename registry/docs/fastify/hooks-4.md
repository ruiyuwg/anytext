...
[Request](/docs/v3.29.x/Reference/Request/.md) and [Reply](/docs/v3.29.x/Reference/Reply/.md) are the core Fastify objects.

`done` is the function to continue with the [lifecycle](/docs/v3.29.x/Reference/Lifecycle/.md).

It is easy to understand where each hook is executed by looking at the [lifecycle page](/docs/v3.29.x/Reference/Lifecycle/.md).
...
*The error will be handled by [`Reply`](/docs/v3.29.x/Reference/Reply/.md#errors).*
...
Triggered when `fastify.close()` is invoked to stop the server. It is useful when [plugins](/docs/v3.29.x/Reference/Plugins/.md) need a "shutdown" event, for example, to close an open connection to a database.
...
Except for [onClose](#onclose), all hooks are encapsulated. This means that you can decide where your hooks should run by using `register` as explained in the [plugins guide](/docs/v3.29.x/Guides/Plugins-Guide/.md). If you pass a function, that function is bound to the right Fastify context and from there you have full access to the Fastify API.
