...
[Request](/docs/v4.29.x/Reference/Request/.md) and [Reply](/docs/v4.29.x/Reference/Reply/.md) are the core Fastify objects.

`done` is the function to continue with the [lifecycle](/docs/v4.29.x/Reference/Lifecycle/.md).

It is easy to understand where each hook is executed by looking at the [lifecycle page](/docs/v4.29.x/Reference/Lifecycle/.md).
...
**Notice:** The size of the returned stream is checked to not exceed the limit set in [`bodyLimit`](/docs/v4.29.x/Reference/Server/.md#bodylimit) option.
...
This hook will be executed only after the [Custom Error Handler set by `setErrorHandler`](/docs/v4.29.x/Reference/Server/.md#seterrorhandler) has been executed, and only if the custom error handler sends an error back to the user *(Note that the default error handler always sends the error back to the user)*.
...
**Notice:** client abort detection is not completely reliable. See: [`Detecting-When-Clients-Abort.md`](/docs/v4.29.x/Guides/Detecting-When-Clients-Abort/.md)
...
*The error will be handled by [`Reply`](/docs/v4.29.x/Reference/Reply/.md#errors).*
...
Triggered when `fastify.close()` is invoked to stop the server, after all in-flight HTTP requests have been completed. It is useful when [plugins](/docs/v4.29.x/Reference/Plugins/.md) need a "shutdown" event, for example, to close an open connection to a database.
...
Triggered when `fastify.close()` is invoked to stop the server, before all in-flight HTTP requests have been completed. It is useful when [plugins](/docs/v4.29.x/Reference/Plugins/.md) have set up some state attached to the HTTP server that would prevent the server to close. *It is unlikely you will need to use this hook*, use the [`onClose`](#onclose) for the most common case.
...
Triggered when a new route is registered. Listeners are passed a [`routeOptions`](/docs/v4.29.x/Reference/Routes/.md#routes-options) object as the sole parameter. The interface is synchronous, and, as such, the listeners are not passed a callback. This hook is encapsulated.
...
Except for [onClose](#onclose), all hooks are encapsulated. This means that you can decide where your hooks should run by using `register` as explained in the [plugins guide](/docs/v4.29.x/Guides/Plugins-Guide/.md). If you pass a function, that function is bound to the right Fastify context and from there you have full access to the Fastify API.
...
Regarding TypeScript in this example, you'd need to update the `FastifyRequest` core interface to include your new property typing (for more about it, see [TypeScript](/docs/v4.29.x/Reference/TypeScript/.md) page), like:
...
Although this is a very pragmatic approach, if you're trying to do something more complex that changes these core objects, then consider creating a custom [Plugin](/docs/v4.29.x/Reference/Plugins/.md) instead.
