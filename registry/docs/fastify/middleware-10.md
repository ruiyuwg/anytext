...
Middleware can be encapsulated, allowing control over where it runs using `register` as explained in the [plugins guide](/docs/v5.5.x/Guides/Plugins-Guide/.md).

Fastify middleware does not expose the `send` method or other methods specific to the Fastify [Reply](/docs/v5.5.x/Reference/Reply/.md#reply) instance. This is because Fastify wraps the incoming `req` and `res` Node instances using the [Request](/docs/v5.5.x/Reference/Request/.md#request) and [Reply](/docs/v5.5.x/Reference/Reply/.md#reply) objects internally, but this is done after the middleware phase. To create middleware, use the Node `req` and `res` instances. Alternatively, use the `preHandler` hook that already has the Fastify [Request](/docs/v5.5.x/Reference/Request/.md#request) and [Reply](/docs/v5.5.x/Reference/Reply/.md#reply) instances. For more information, see [Hooks](/docs/v5.5.x/Reference/Hooks/.md#hooks).
