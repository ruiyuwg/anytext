...
From the [Hooks documentation](/docs/v5.5.x/Reference/Hooks/.md#manage-errors-from-a-hook):

> If you get an error during the execution of your hook, just pass it to `done()` and Fastify will automatically close the request and send the appropriate error code to the user.

When a custom error handler is defined through [`setErrorHandler`](/docs/v5.5.x/Reference/Server/.md#seterrorhandler), it will receive the error passed to the `done()` callback or through other supported automatic error handling mechanisms. If `setErrorHandler` is used multiple times, the error will be routed to the most precedent handler within the error [encapsulation context](/docs/v5.5.x/Reference/Encapsulation/.md). Error handlers are fully encapsulated, so a `setErrorHandler` call within a plugin will limit the error handler to that plugin's context.
...
- `reply.send(data)` behaves as in [regular route handlers](/docs/v5.5.x/Reference/Reply/.md#senddata)
...
When using Fastify's custom error handling through [`setErrorHandler`](/docs/v5.5.x/Reference/Server/.md#seterrorhandler), be aware of how errors are propagated between custom and default error handlers.
