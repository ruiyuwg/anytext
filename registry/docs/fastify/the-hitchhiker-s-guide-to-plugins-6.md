...
Your routes, your utilities, and so on are all plugins. To add a new plugin, whatever its functionality may be, in Fastify you have a nice and unique API: [`register`](/docs/v5.0.x/Reference/Plugins/.md).
...
Fastify offers you a more elegant and comfortable way to do this, *decorators*. Creating a decorator is extremely easy, just use the [`decorate`](/docs/v5.0.x/Reference/Decorators/.md) API:
...
We have seen how to extend server functionality and how to handle the encapsulation system, but what if you need to add a function that must be executed whenever the server "[emits](/docs/v5.0.x/Reference/Lifecycle/.md)" an event?
...
So what can you do to avoid this annoying issue? Yes, you are right, use a [hook](/docs/v5.0.x/Reference/Hooks/.md)!
...
An alternative approach is to make use of the [onRoute hook](/docs/v5.0.x/Reference/Hooks/.md#onroute) to customize application routes dynamically from inside the plugin. Every time a new route is registered, you can read and modify the route options. For example, based on a [route config option](/docs/v5.0.x/Reference/Routes/.md#routes-options):
