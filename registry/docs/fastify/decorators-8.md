...
This API is *synchronous*. Defining a decoration asynchronously could result in the Fastify instance booting before the decoration completes. To register an asynchronous decoration, use the `register` API with `fastify-plugin`. See the [Plugins](/docs/v5.2.x/Reference/Plugins/.md) documentation for more details.
...
This method customizes the Fastify [server](/docs/v5.2.x/Reference/Server/.md) instance.
...
The decorated [Fastify server](/docs/v5.2.x/Reference/Server/.md) is bound to `this` in [route](/docs/v5.2.x/Reference/Routes/.md) handlers:
...
To achieve proper encapsulation across requests configure a new value for each incoming request in the [`'onRequest'` hook](/docs/v5.2.x/Reference/Hooks/.md#onrequest).
