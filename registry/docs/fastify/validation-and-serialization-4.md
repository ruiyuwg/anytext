...
Validation will only be attempted if the content type is `application-json`, as described in the documentation for the [content type parser](/docs/v3.29.x/Reference/ContentTypeParser/.md).
...
The shared schemas can be reused through the JSON Schema [**`$ref`**](https://tools.ietf.org/html/draft-handrews-json-schema-01#section-8) keyword. Here an overview of *how* references work:
...
The route validation internally relies upon [Ajv v6](https://www.npmjs.com/package/ajv/v/6.12.6) which is a high-performance JSON Schema validator. Validating the input is very easy: just add the fields that you need inside the route schema, and you are done!
...
> ℹ If you need to use the lastest version of Ajv (v8) you should read how to do it in the [`schemaController`](/docs/v3.29.x/Reference/Server/.md#schema-controller) section. It is explained the easier way to avoid to implement a custom validator.
...
The Ajv default configuration in Fastify doesn't support coercing array parameters in querystring. However, Fastify allows [`customOptions`](/docs/v3.29.x/Reference/Server/.md#ajv) in Ajv instance. The `coerceTypes: 'array'` will coerce one parameter to a single element in array. Example:
...
You can provide a list of plugins you want to use with the default `ajv` instance. Note that the plugin must be **compatible with Ajv v6**.

> Refer to [`ajv options`](/docs/v3.29.x/Reference/Server/.md#ajv) to check plugins format
...
This baseline configuration can be modified by providing [`ajv.customOptions`](/docs/v3.29.x/Reference/Server/.md#factory-ajv) to your Fastify factory.
...
*If you need a custom serializer in a very specific part of your code, you can set one with [`reply.serializer(...)`](/docs/v3.29.x/Reference/Reply/.md#serializerfunc).*
...
You can also use [setErrorHandler](/docs/v3.29.x/Reference/Server/.md#seterrorhandler) to define a custom response for validation errors such as
