...
Fastify can be extended with plugins, which can be a set of routes, a server [decorator](/docs/v5.5.x/Reference/Decorators/.md), or other functionality. Use the `register` API to add one or more plugins.
...
The [Getting Started](/docs/v5.5.x/Guides/Getting-Started/.md#your-first-plugin) guide includes an example of using this API:
...
- [`logLevel`](/docs/v5.5.x/Reference/Routes/.md#custom-log-level)
- [`logSerializers`](/docs/v5.5.x/Reference/Routes/.md#custom-log-serializer)
...
If an option with the key `prefix` and a `string` value is passed, Fastify will use it to prefix all the routes inside the register. For more info, check [here](/docs/v5.5.x/Reference/Routes/.md#route-prefixing).

Be aware that if routes are wrapped with [`fastify-plugin`](https://github.com/fastify/fastify-plugin), this option will not work (see the [workaround](/docs/v5.5.x/Reference/Routes/.md#fastify-plugin)).
...
If `register` is used only to extend server functionality with [`decorate`](/docs/v5.5.x/Reference/Decorators/.md), tell Fastify not to create a new scope. Otherwise, changes will not be accessible in the upper scope.
