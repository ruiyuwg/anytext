...
- `query` - The parsed querystring, its format is specified by [`querystringParser`](/docs/v5.6.x/Reference/Server/.md#querystringparser).

- `body` - The request payload, see [Content-Type Parser](/docs/v5.6.x/Reference/ContentTypeParser/.md) for details on what request payloads Fastify natively parses and how to support other content types.
...
- `server` - The Fastify server instance, scoped to the current [encapsulation context](/docs/v5.6.x/Reference/Encapsulation/.md).
...
- `ips` - An array of the IP addresses, ordered from closest to furthest, in the `X-Forwarded-For` header of the incoming request (only when the [`trustProxy`](/docs/v5.6.x/Reference/Server/.md#factory-trust-proxy) option is enabled).

- `host` - The host of the incoming request (derived from `X-Forwarded-Host` header when the [`trustProxy`](/docs/v5.6.x/Reference/Server/.md#factory-trust-proxy) option is enabled). For HTTP/2 compatibility, it returns `:authority` if no host header exists. The host header may return an empty string if `requireHostHeader` is `false`, not provided with HTTP/1.0, or removed by schema validation.
...
- `context` - Deprecated, use `request.routeOptions.config` instead. A Fastify internal object. Do not use or modify it directly. It is useful to access one special key:
  ...
  - `context.config` - The route [`config`](/docs/v5.6.x/Reference/Routes/.md#routes-config) object.

- `routeOptions` - The route [`option`](/docs/v5.6.x/Reference/Routes/.md#routes-options) object.

  - `bodyLimit` - Either server limit or route limit.
  - `config` - The [`config`](/docs/v5.6.x/Reference/Routes/.md#routes-config) object for this route.
