...
Remember this example works only with value types as reference types will thrown and error during the fastify startup. See [decorateRequest](#decorate-request).

See [JavaScript engine fundamentals: Shapes and Inline Caches](https://mathiasbynens.be/notes/shapes-ics) for more information on this topic.
...
This method is used to customize the Fastify [server](/docs/v5.1.x/Reference/Server/.md) instance.
...
The decorated [Fastify server](/docs/v5.1.x/Reference/Server/.md) is bound to `this` in [route](/docs/v5.1.x/Reference/Routes/.md) handlers:
...
To achieve proper encapsulation across requests configure a new value for each incoming request in the [`'onRequest'` hook](/docs/v5.1.x/Reference/Hooks/.md#onrequest). Example:
...
As above with [`decorateReply`](#decorate-reply), this API is used add new methods/properties to the core `Request` object:
...
To achieve proper encapsulation across requests configure a new value for each incoming request in the [`'onRequest'` hook](/docs/v5.1.x/Reference/Hooks/.md#onrequest).
