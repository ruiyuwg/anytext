...
Fastify automatically adds the parsed request payload to the [Fastify request](/docs/v5.8.x/Reference/Request/.md) object, accessible via `request.body`.

> **Important:** When using a body schema with the [`content`](/docs/v5.8.x/Reference/Validation-and-Serialization/.md#body-content-type-validation) property to validate per content type, only content types listed in the schema will be validated. If you add a custom content type parser but do not include its content type in the body schema's `content` property, the incoming data will be parsed but **not validated**.
...
The request body can be parsed in two ways. First, add a custom content type parser and handle the request stream. Or second, use the `parseAs` option in the `addContentTypeParser` API, specifying `'string'` or `'buffer'`. Fastify will handle the stream, check the [maximum size](/docs/v5.8.x/Reference/Server/.md#factory-body-limit) of the body, and the content length. If the limit is exceeded, the custom parser will not be invoked.
...
- `bodyLimit` (number): The maximum payload size, in bytes, that the custom parser will accept. Defaults to the global body limit passed to the [`Fastify factory function`](/docs/v5.8.x/Reference/Server/.md#bodylimit).
