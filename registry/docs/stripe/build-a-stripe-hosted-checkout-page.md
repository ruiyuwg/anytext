...
Configure the copy displayed on the Checkout submit button by setting the [submit\_type](https://docs.stripe.com/api/checkout/sessions/create.md#create_checkout_session-submit_type). There are four different submit types.
...
mode: {{CHECKOUT\_MODE}},
success\_url: `${YOUR_DOMAIN}/success.html`,
success\_url: `${YOUR_DOMAIN}?success=true`,
...
mode: {{CHECKOUT\_MODE}},
success\_url: `${origin}/success?session_id={CHECKOUT_SESSION_ID}`,
automatic\_tax: {enabled: true},
});
...
if (!session\_id)
throw new Error('Please provide a valid session\_id (`cs_test_...`)')
