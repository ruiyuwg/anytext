...
Available in: US
Instant Bank Payments let your customers pay with a US bank account using Link. Unlike [ACH Direct Debit](https://docs.stripe.com/payments/ach-direct-debit.md), Instant Bank Payments deliver instant confirmation and faster settlement, protection from common ACH failures, and accelerated checkout.
...
- **Transaction limits**: Eligible transactions must be below an amount dynamically set by our risk systems. By default, Instant Bank Payments are presented only for transactions under 5000 USD.
