...
- **Customer locations**
  ...
  [Varies by merchant country](https://docs.stripe.com/payments/bank-transfers.md#bank-transfer-methods)
...
Bank transfers users in the following regions can accept international wire transfers (SWIFT):

- The United States

International wire transfers might incur fees on the way to Stripe, which can result in an amount received that’s less than what the customer originally sent. Stripe-incurred fees appear on the balances page in the Dashboard, alongside other relevant Stripe fees. The amount shown in the cash balance is the amount that Stripe received from the customer.
...
2. In the **Payment Methods** section, click Add (+) and choose **Add a bank transfer account**.
3. In the editor, choose the currency to charge the customer in for their bank transfer payments.
...
The [on\_behalf\_of attribute](https://docs.stripe.com/api/payment_intents/object.md#payment_intent_object-on_behalf_of) isn’t supported.
